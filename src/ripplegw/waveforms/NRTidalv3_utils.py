...
def _get_merger_frequency(theta: Float[Array, "6"]) -> Float:
    ...
    # convert from angular frequency to frequency (divide by TWO_PI) and then convert from dimensionless frequency to Hz (divide by M * MTSUN)
...
def get_tidal_phase(
    ...
    numA = (
        1.0
        + (n_1A * PN_x)
        + (n_3over2A * PN_x_3over2)
        + (n_2A * PN_x_2)
        + (n_5over2A * PN_x_5over2)
        + (n_3A * PN_x_3)
    )
    ...
    numB = (
        1.0
        + (n_1B * PN_x)
        + (n_3over2B * PN_x_3over2)
        + (n_2B * PN_x_2)
        + (n_5over2B * PN_x_5over2)
        + (n_3B * PN_x_3)
    )
...
def get_tidal_phase_PN(
    ...
    tidal_phasePNA = factorA * (
        1.0
        + (c_1A * PN_x)
        + (c_3over2A * PN_x_3over2)
        + (c_2A * PN_x_2)
        + (c_5over2A * PN_x_5over2)
    )
    tidal_phasePNB = factorB * (
        1.0
        + (c_1B * PN_x)
        + (c_3over2B * PN_x_3over2)
        + (c_2B * PN_x_2)
        + (c_5over2B * PN_x_5over2)
    )
...
def get_tidalphasePN_coeffs(theta_intrinsic: Float[Array, "6"]) -> Float[Array, "10"]:
    ...
    PN_coeffs1 = (-1300.0 * Xa_3 + 11430.0 * Xa_2 + 4595.0 * Xa - 15895.0) / (
        672.0 * den_a
    )  # -5.0*(260.0*Xa_3 - 2286.0*Xa_2 - 919.0*Xa + 3179.0)/(672.0*(11.0*Xa-12.0))
    PN_coeffs2 = -PI
    PN_coeffs3 = (
        (
            22861440.0 * Xa_5
        ...
        / (27433728.0 * den_a)
    )  # (5.0*(4572288.0*Xa_5 - 20427120.0*Xa_4 + 158378220.0*Xa_3 +174965616.0*Xa_2 + 43246839.0*Xa -387973870.0))/(27433728.0*(11.0*Xa - 12.0))
    PN_coeffs4 = (
        -PI
        * (10520.0 * Xa_3 - 7598.0 * Xa_2 + 22415.0 * Xa - 27719.0)
        / (672.0 * den_a)
    )

    PN_coeffs5 = -3.0 * den_b / (16 * Xb * Xa_2)  # (3.0)*(12.0 -11.0*Xb)/(16*Xb*Xa_2)
    PN_coeffs6 = (-1300.0 * Xb_3 + 11430.0 * Xb_2 + 4595.0 * Xb - 15895.0) / (
        672.0 * den_b
    )  # -5.0*(260.0*Xb_3 - 2286.0*Xb_2 - 919.0*Xb + 3179.0)/(672.0*(11.0*Xb-12.0))
    PN_coeffs7 = -PI
    PN_coeffs8 = (
        (
            22861440.0 * Xb_5
        ...
        / (27433728.0 * den_b)
    )  # (5.0*(4572288.0*Xb_5 - 20427120.0*Xb_4 + 158378220.0*Xb_3 +174965616.0*Xb_2 + 43246839.0*Xb -387973870.0))/(27433728.0*(11.0*Xb - 12.0))
    PN_coeffs9 = (
        -PI
        * (10520.0 * Xb_3 - 7598.0 * Xb_2 + 22415.0 * Xb - 27719.0)
        / (672.0 * den_b)
    )
...
def get_NRTidalv3_coefficients(
    ...
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[1].set(
        s20 + s21 * kappa2T + s22 * q * kappa2T
    )
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[2].set(
        s30 + s31 * kappa2T + s32 * q * kappa2T
    )

    s2s3 = NRTidalv3_coeffs[1] * NRTidalv3_coeffs[2]
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[3].set(jnp.cosh(s2s3) + jnp.sinh(s2s3))

    NRTidalv3_coeffs = NRTidalv3_coeffs.at[4].set(
        3.0 * Xb * Xa * Xa * Xa * Xa * lambda1
    )  # kappaA
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[5].set(
        3.0 * Xa * Xb * Xb * Xb * Xb * lambda2
    )  # kappaB

    # Exponent parameters:
    alpha = -8.08155404e-03  # alpha
    beta = -1.13695919e00  # beta
    ...
    n_5over20 = -9.40654388e02  # n_5over20
    n_5over21 = 6.26517157e02  # n_5over21
    n_5over22 = 5.53629706e02  # n_5over22
    n_5over23 = 8.84823087e01  # n_5over23

    n_30 = 4.05483848e02  # n_30
    n_31 = -4.25525054e02  # n_31
    n_32 = -1.92004957e02  # n_32
    n_33 = -5.10967553e01  # n_33

    d_10 = 3.80343306e00  # d_10
    d_11 = -2.52026996e01  # d_11
    d_12 = -3.08054443e00  # d_12

    NRTidalv3_coeffs = NRTidalv3_coeffs.at[6].set(
        n_5over20 + n_5over21 * Xa + n_5over22 * kappaA_alpha + n_5over23 * Xa_beta
    )
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[7].set(
        n_30 + n_31 * Xa + n_32 * kappaA_alpha + n_33 * Xa_beta
    )
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[8].set(d_10 + d_11 * Xa + d_12 * Xa_beta)

    NRTidalv3_coeffs = NRTidalv3_coeffs.at[9].set(
        n_5over20 + n_5over21 * Xb + n_5over22 * kappaB_alpha + n_5over23 * Xb_beta
    )
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[10].set(
        n_30 + n_31 * Xb + n_32 * kappaB_alpha + n_33 * Xb_beta
    )
    ...
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[13].set(
        (
            (c_1A * c_3over2A)
            - c_5over2A
    ...
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[15].set(
        -(c_5over2A + c_3over2A * NRTidalv3_coeffs[8] - NRTidalv3_coeffs[6]) * inv_c1_A
    )

    inv_c1_B = 1.0 / c_1B
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[16].set(c_1B + NRTidalv3_coeffs[11])
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[17].set(
        (
            (c_1B * c_3over2B)
            - c_5over2B
            - (c_3over2B) * NRTidalv3_coeffs[11]
            + NRTidalv3_coeffs[9]
        )
        * inv_c1_B
    )
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[18].set(c_2B + c_1B * NRTidalv3_coeffs[11])
    NRTidalv3_coeffs = NRTidalv3_coeffs.at[19].set(
        -(c_5over2B + c_3over2B * NRTidalv3_coeffs[11] - NRTidalv3_coeffs[9]) * inv_c1_B
    )
