...
# Mode mixing coefficients for the 32 mode (spheroidal-spherical decomposition)


def evaluate_QNMfit_re_l2m2lp2(a: float) -> float:
    """Re(mu_{l=2,m=2,lp=2}) mixing coefficient."""
    x2 = a * a
    x3 = x2 * a
    x4 = x2 * x2
    x5 = x3 * x2
    ...
    num = (
        1.0
        - 2.2956993576253635 * a
        + 1.461988775298876 * x2
        + 0.0043296365593147035 * x3
        - 0.1695667458204109 * x4
        - 0.0006267849034466508 * x5
    )
    den = (
        1.0
        - 2.2956977727459043 * a
        + 1.4646339137818438 * x2
        - 0.16843226886562457 * x4
        - 0.00007150540890128118 * x6
    )
    return num / den


def evaluate_QNMfit_im_l2m2lp2(a: float) -> float:
    """Im(mu_{l=2,m=2,lp=2}) mixing coefficient."""
    x2 = a * a
    x3 = x2 * a
    x4 = x2 * x2
    x5 = x3 * x2
    ...
    num = a * (
        0.3826673013161342
        - 0.47531267226013896 * a
        - 0.05898102880105067 * x2
        + 0.0724525431346487 * x3
        + 0.054714637311702986 * x4
        + 0.024544862718252784 * x5
    )
    den = (
        -38.70835035062785
        + 69.82140084545878 * a
        - 27.99036444363243 * x2
        - 4.152310472191899 * x4
        + x6
    )
    return num / den


def evaluate_QNMfit_re_l3m2lp2(a: float) -> float:
    """Re(mu_{l=3,m=2,lp=2}) mixing coefficient."""
    x2 = a * a
    x3 = x2 * a
    x4 = x2 * x2
    x5 = x3 * x2
    ...
    den = -6.76894063440646 + 15.170831931186493 * a - 9.406169787571082 * x2 + x4
    return num / den


def evaluate_QNMfit_im_l3m2lp2(a: float) -> float:
    """Im(mu_{l=3,m=2,lp=2}) mixing coefficient."""
    x2 = a * a
    x3 = x2 * a
    x4 = x2 * x2
    x5 = x3 * x2
...
def evaluate_QNMfit_re_l2m2lp3(a: float) -> float:
    """Re(mu_{l=2,m=2,lp=3}) mixing coefficient."""
    x2 = a * a
    x3 = x2 * a
    x4 = x2 * x2
    x5 = x3 * x2
    ...
    num = a * (
        18.522563276099167
        - 37.978140351289014 * a
        + 19.030390708998894 * x2
        + 3.0355668591803386 * x3
        - 2.210028290847915 * x4
        - 0.37117112862247975 * x5
    )
    den = (
        164.52480238697507
        - 377.9093045285145 * a
        + 243.3353695550844 * x2
        - 30.79738566181734 * x4
        + x6
    )
    return num / den


def evaluate_QNMfit_im_l2m2lp3(a: float) -> float:
    """Im(mu_{l=2,m=2,lp=3}) mixing coefficient."""
    x2 = a * a
    x3 = x2 * a
    x4 = x2 * x2
    x5 = x3 * x2
    ...
    num = a * (
        -49.7688437256778
        + 120.43773704442333 * a
        - 82.95323455645332 * x2
        + 1.721453011852496 * x3
        + 11.540237244397877 * x4
        - 0.9819458637589314 * x5
    )
    den = (
        2858.5790831181725
        - 6305.619505422591 * a
        + 3825.6742092829054 * x2
        - 377.7822297815406 * x4
        + x6
    )
    return num / den


def evaluate_QNMfit_re_l3m2lp3(a: float) -> float:
    """Re(mu_{l=3,m=2,lp=3}) mixing coefficient."""
    x2 = a * a
    x3 = x2 * a
    x4 = x2 * x2
    x5 = x3 * x2
...
def evaluate_QNMfit_im_l3m2lp3(a: float) -> float:
    """Im(mu_{l=3,m=2,lp=3}) mixing coefficient."""
    x2 = a * a
    x3 = x2 * a
    x4 = x2 * x2
    x5 = x3 * x2
...
def IMRPhenomX_TimeShift_22(pWF22: dict) -> float:
    ...
    # dphi22Ref = d(Phase_XAS)/df_Hz / M_s = d(Phase_XAS)/d(Mf)
    # IMRPhenomXAS_Phase already includes the 1/eta factor
    dphi22Ref = jax.grad(IMRPhenomXAS_Phase)(frefFit_Hz, theta, phase_coeffs) / M_s
...
def _xhm_get_spheroidal_coeffs(
    ...
    dphi22_frefTS = (
        jax.grad(lambda Mf: IMRPhenomXAS_Phase(Mf / M_s, theta, phase_coeffs))(frefTS)
        + t0
    )
    ...
    frefPS = fRING22
    phi22_frefPS = (
        IMRPhenomXAS_Phase(frefPS / M_s, theta, phase_coeffs) + t0 * frefPS + phifRef
    )
...
def xhm_phase_ModeMixing(
    ...
    """
    Evaluate the (3,2) mode phase using spheroidal mode mixing in the ringdown.

    Mixing combines 22 and 32 contributions via complex mu coefficients from QNM fits.
    Source: IMRPhenomXHM_Phase_ModeMixing in LALSimIMRPhenomXHM.c.
    Note: deferred to second implementation pass (see plan §6).
    """
    raise NotImplementedError("32-mode mixing phase deferred to second pass")
...
def _xhm_rd_rescaled(
    ...
    """
    RD amplitude ansatz case 0 (122018, without f^(-7/6)*ampNorm).
    Returns: fDAMP*|alambda|*sigma * exp(-lambda*(f-fRING)/(fDAMP*sigma)) / ((f-fRING)^2+(fDAMP*sigma)^2) * f^(-1/12)
    """
    dfr = f - fRING
    dfd = fDAMP * sigma
    lc = 1.0 / 12.0
...
# ── Intermediate amplitude polynomial helpers (LAL 122019 closed forms) ─────


def _inter_delta_101(v1, v4, f1, f4):
    f1mf4 = f1 - f4
    d0 = (-f4 * v1 + f1 * v4) / f1mf4
    d1 = (v1 - v4) / f1mf4
    return jnp.array([d0, d1, 0.0, 0.0, 0.0, 0.0])


def _inter_delta_102(d1c, d4c, v1, v4, f1, f4):
    f12 = f1 * f1
    f42 = f4 * f4
    f1mf4 = f1 - f4
    f1mf42 = f1mf4 * f1mf4
    dd0 = (-(d4c * f1 * f1mf4 * f4) + f42 * v1 + f12 * v4 - 2 * f1 * f4 * v4) / f1mf42
    dd1 = (d4c * (f12 - f42) + 2 * f4 * (-v1 + v4)) / f1mf42
    dd2 = (-(d4c * f1mf4) + v1 - v4) / f1mf42
    return jnp.array([dd0, dd1, dd2, 0.0, 0.0, 0.0])


def _inter_delta_1032(d1c, d4c, v1, v4, f1, f4):
    f12 = f1 * f1
    f13 = f12 * f1
    f42 = f4 * f4
    f43 = f42 * f4
    ...
    dd1 = (
        d4c * f1 * f1mf4 * (f1 + 2 * f4)
        - f4 * (d1c * (-2 * f12 + f1 * f4 + f42) + 6 * f1 * (v1 - v4))
    ) / f1mf43
    dd2 = (
        -(d1c * (f12 + f1 * f4 - 2 * f42))
        + d4c * (-2 * f12 + f1 * f4 + f42)
    ...
    dd3 = (d1c * f1mf4 + d4c * f1mf4 - 2 * v1 + 2 * v4) / f1mf43
    return jnp.array([dd0, dd1, dd2, dd3, 0.0, 0.0])


def _inter_delta_1042(d1c, d4c, v1, v3, v4, f1, f3, f4):
    """4th order poly (version 1042): v1,d1, single middle at (f3,v3), v4,d4."""
    f12 = f1 * f1
    f13 = f12 * f1
    f14 = f13 * f1
    f15 = f14 * f1
    ...
    f3mf42 = f3mf4 * f3mf4
    f1mf43 = f1mf42 * f1mf4
    denom = f1mf32 * f1mf43 * f3mf42

    dd0 = (
        -(d4c * f12 * f1mf32 * f1mf4 * f3 * f3mf4 * f4)
        + d1c * f1 * f1mf3 * f1mf4 * f3 * f3mf42 * f42
        - 4 * f12 * f33 * f42 * v1
        + 3 * f1 * f34 * f42 * v1
        + 8 * f12 * f32 * f43 * v1
        - 4 * f1 * f33 * f43 * v1
    ...
    dd2 = (
        -(d4c * f1mf32 * f1mf4 * f3mf4 * (f12 + f3 * f4 + 2 * f1 * (f3 + f4)))
        + d1c * f1mf3 * f1mf4 * f3mf42 * (f1 * f3 + 2 * (f1 + f3) * f4 + f42)
        - 4 * f12 * f33 * v1
        + 3 * f1 * f34 * v1
        - 4 * f1 * f33 * f4 * v1
        + 3 * f34 * f4 * v1
    ...
    dd4 = (
        -(d4c * f1mf32 * f1mf4 * f3mf4)
        + d1c * f1mf3 * f1mf4 * f3mf42
        - 3 * f1 * f32 * v1
        + 2 * f33 * v1
        + 6 * f1 * f3 * f4 * v1
        - 3 * f32 * f4 * v1
    ...
    return jnp.array([dd0, dd1, dd2, dd3, dd4, 0.0])


def _inter_cross_zero(d, f1, f4, degree, n=64):
    """True if polynomial sum_{k=0}^{degree} d[k]*f^k has sign change in [f1, f4]."""
    probe = jnp.linspace(f1, f4, n)
    vals = jnp.zeros(n)
    for k in range(degree + 1):
        vals = vals + d[k] * probe**k
    return (jnp.min(vals) * jnp.max(vals)) < 0.0


def _xhm_21_spa_coeffs(
    mm1: float, mm2: float, chi1: float, chi2: float, eta: float, delta: float
) -> tuple:
    """
    SPA (Stationary Phase Approximation) amplitude coefficients for mode (2,1).
    Source: Get21PNAmplitudeCoefficients in LALSimIMRPhenomXHM_internals.c.
    Uses fractional masses mm1=m1/(m1+m2), mm2=m2/(m1+m2).
    Returns: (PNTDfactor, x05, x1, x15, x2, x25, x3,
               xdot5, xdot6, xdot65, xdot7, xdot75, xdot8, xdot8Log, xdot85,
               log2pi_two_thirds)
    """
    PI = jnp.pi
    chiS = 0.5 * (chi1 + chi2)
    ...
    chi12 = chi1 * chi1
    chi22 = chi2 * chi2
    Sc = mm1**2 * chi1 + mm2**2 * chi2
    Sigmac = mm2 * chi2 - mm1 * chi1
    m1 = mm1
    m2 = mm2
    m12 = m1 * m1
    m22 = m2 * m2
    ...
    PNTDfactor = 8.0 * eta * (2 * PI) ** (2.0 / 3) * jnp.sqrt(PI) / jnp.sqrt(5.0)

    p13 = 2.0 ** (1.0 / 3)
    p23 = 2.0 ** (2.0 / 3)
    p43 = 2.0 ** (4.0 / 3)
    p53 = 2.0 ** (5.0 / 3)
    x05 = 1j * delta / 3.0 * p13 * PI ** (1.0 / 3)
    x1 = -1j * 0.5 * (chiA + chiS * delta) * p23 * PI ** (2.0 / 3)
    x15 = 1j * delta * (-17.0 / 28.0 + 5.0 * eta / 7.0) / 3.0 * 2.0 * PI
    x2 = (
        (
            1j * (-43.0 / 21.0 * delta * Sc + (-79.0 + 139.0 * eta) / 42.0 * Sigmac)
            + 1j / 3.0 * delta * (PI + 1j * (-0.5 - 2.0 * logof2))
        )
        * p43
        * PI ** (4.0 / 3)
    )
    x25 = (
        1j
        * delta
        * ((-43.0 - 509.0 * eta) / 126.0 + 79.0 * eta**2 / 168.0)
        / 3.0
        * p53
        * PI ** (5.0 / 3)
    )
    ...
    denom = 3.274425e7
    twopi_10_3 = (2 * PI) ** (10.0 / 3)
    twopi_4 = (2 * PI) ** 4
    twopi_13_3 = (2 * PI) ** (13.0 / 3)
    twopi_14_3 = (2 * PI) ** (14.0 / 3)
    twopi_15_3 = (2 * PI) ** 5
    twopi_16_3 = (2 * PI) ** (16.0 / 3)
    twopi_17_3 = (2 * PI) ** (17.0 / 3)
    ...
    xdot65 = (
        -(
            ...
            * (
                ...
                * m1
                * m2
                * (-(m2 * (75.0 * chi1 + 376.0 * chi2 * m2)) + 96.0 * PI)
                - 34927200.0
                * (-(m2 * (75.0 * chi2 + 188.0 * (chi1 + chi2) * m2)) + 48.0 * PI)
                * m12
                - 2619540000.0 * chi1 * m13
                + 13132627200.0 * chi1 * m2 * m13
                + 6566313600.0 * chi1 * m14
                - 34927200.0 * (chi2 * (75.0 - 188.0 * m2) * m2 + 48.0 * PI) * m22
            )
            / denom
        )
        * twopi_13_3
    ...
    xdot75 = (
        -(
            ...
            * (
                ...
                + 5821200.0 * m2 * (5861.0 * chi1 + 1701.0 * PI) * m13
                + 17059026600.0 * chi1 * m14
                + 14721814800.0 * chi1 * m2 * m14
                - 34962127200.0 * chi1 * m2 * m15
                + 207900.0
                * (2.0 * chi2 * m2 * (-9708.0 + 41027.0 * m2) + 12477.0 * PI)
                * m22
    ...
    xdot8 = (
        -(
            ...
            * m2
            * (
                -10548014400.0 * chi1 * PI * m13
                - 63392868000.0 * chi1 * chi2 * m2 * m14
                + 34927200.0 * chi1 * (-375.0 * chi1 + 752.0 * PI) * m14
                ...
                * (
                    ...
                    - 23284800.0 * chi2 * m2 * (-151.0 + 376.0 * m2) * PI
                    - 2277918720.0 * log4
                    + 2321480700.0 * chi22
    ...
    xdot8Log = -(m1 * m2 * 3416878080.0) / denom * twopi_16_3
    ...
    xdot85 = (
        -(
            ...
            * (
                ...
                + 1925.0
                * m2
                * (97151928.0 * chi1 + 6613488.0 * chi2 - 12912300.0 * PI)
                * m13
                + 87143248500.0 * chi1 * m14
                + 33313480200.0 * chi1 * m2 * m14
                - 198816225300.0 * chi1 * m2 * m15
                + 57750.0
                * (2.0 * chi2 * m2 * (-128927.0 + 754487.0 * m2) + 7947.0 * PI)
                * m22
                - 33313480200.0 * chi2 * m13 * m22
                - 138600.0
                * (3399633.0 * chi1 + 530712.0 * chi2 + 182990.0 * PI)
                * m14
                * m22
                ...
                * (
                ...
                - 69300.0 * (4991769.0 * (chi1 + chi2) + 731960.0 * PI) * m13 * m23
                ...
                * m12
                * (
                    15.0 * PI * (-2649.0 + 286940.0 * m22 + 146392.0 * m24)
                    + 2.0
                    * m2
                    * (
                        -644635.0 * chi2
    ...
    log2pi_two_thirds = jnp.log((2 * PI) ** (2.0 / 3))
...
def _xhm_21_spa_amp(Mf: float, spa_coeffs: tuple) -> float:
    """
    SPA inspiral amplitude for mode (2,1) at InspRescaleFactor=1.
    Source: IMRPhenomXHM_Inspiral_PNAmp_21Ansatz in LALSimIMRPhenomXHM_inspiral.c.
    """
    (
        PNTDfactor,
        x05,
    ...
    CpnAmpTD_mag = jnp.abs(PNTDfactor * Mf ** (2.0 / 3) * series)
    ...
    XdotT4 = (
        ...
        + Mf**5 * xdot75
        + Mf ** (16.0 / 3)
        * (xdot8 + xdot8Log * (2.0 / 3 * jnp.log(Mf) + log2pi_two_thirds))
        + Mf ** (17.0 / 3) * xdot85
    )

    two_to_m_one_sixths = 2.0 ** (-1.0 / 6)
    three_to_m_one_second = 3.0 ** (-0.5)
    x_to_m_one_four = two_to_m_one_sixths * PI ** (-1.0 / 6) * Mf ** (-1.0 / 6)

    return (
        2.0
        * jnp.sqrt(PI)
        * three_to_m_one_second
        * CpnAmpTD_mag
        * x_to_m_one_four
        / jnp.sqrt(XdotT4)
    )
...
def xhm_get_amp_coefficients(
    ...
    pn_coeffs = _xhm_pn_amp_coeffs(modeTag, eta, delta, chi1L, chi2L)
    spa_coeffs_21 = None  # not used in 122022 non-mixing path
...
def XLALSimIMRPhenomXHMGethlmModes(
    ...
    phi_22_at_ref = IMRPhenomXAS_Phase(Mf_ref / M_s, theta, phase_coeffs)
    ...
    for ell, mm in ell_mm_pairs:
        if ell == 2 and mm == 2:
            ...
            phase_22 = IMRPhenomXAS_Phase(freqs_geom / M_s, theta, phase_coeffs)
            ...
            amp_22 = (
                IMRPhenomXAS_Amp(freqs_geom / M_s, theta, amp_coeffs_22, D=1.0)
                / amp0_1mpc
            )
