...
def IMRPhenomX_Ringdown_Amp_22_gamma2(
    eta: float, S: float, dchi: float, delta: float, RDAmpFlag: int
) -> float:
    """
    ...
    if RDAmpFlag == 103:
        ...
        eqSpin = (
            ...
            * (
                0.5869408584532747
                + eta * (-0.1467158405070222 - 2.8489481072076472 * S)
                + 0.031852563636196894 * S
                + eta2 * (0.25295441250444334 + 4.6849496672664594 * S)
            )
        ) / (3.8775263105069953 - 3.41755361841226 * S + 1.0 * S2)
...
def IMRPhenomX_Ringdown_Amp_22_gamma3(
    eta: float, S: float, dchi: float, delta: float, RDAmpFlag: int
) -> float:
    """
    ...
    if RDAmpFlag == 103:
        noSpin = (
            ...
            - 4.222259944408823 * eta3
        ) / (1.0 - 2.7440263888207594 * eta)

        eqSpin = (
            0.07179105336478316
            + eta2 * (2.331724812782498 - 0.6330998412809531 * S)
            + eta * (-0.8752427297525086 + 0.4168560229353532 * S)
            - 0.05633734476062242 * S
        ) * S

        uneqSpin = 0.0 * delta * dchi  # Explicitly zero in the C code
...
def IMRPhenomX_Ringdown_Amp_22_v1(
    eta: float, S: float, dchi: float, delta: float, RDAmpFlag: int
) -> float:
    """
    ...
    if RDAmpFlag == 103:
        ...
        eqSpin = (
            ...
            * (
                -0.14352506969368556
                + 0.026356911108320547 * S
                + 0.19967405175523437 * S2
                ...
                * (
                    ...
                    - 3.751501972663298 * S
                    + 81.9290740950083 * S2
                    + 30.491948143930266 * S3
                    - 132.77982622925845 * S4
                )
                + eta
                * (
                    -4.805034453745424
                    + 1.11147906765112 * S
                    + 6.176053843938542 * S2
                    - 0.2874540719094058 * S3
                    - 8.990840289951514 * S4
                )
                ...
                * (
                    27.675454081988036
                    - 2.398327419614959 * S
                    - 47.99096500250743 * S2
                    - 5.104257870393138 * S3
                    + 72.08174136362386 * S4
                )
            )
        ) / (-1.4160870461211452 + 1.0 * S)
...
def IMRPhenomX_Ringdown_Amp_22_PeakFrequency(
    gamma2: float, gamma3: float, fRING: float, fDAMP: float, RDAmpFlag: int
) -> float:
    """
    ...
    if RDAmpFlag == 103:
        ...
        if gamma2 <= 1.0:
            fpeak = jnp.abs(
                fRING
                + fDAMP * gamma3 * (jnp.sqrt(1.0 - gamma2 * gamma2) - 1.0) / gamma2
            )
        else:
...
def IMRPhenomX_Ringdown_Phase_22_v4(
    eta: float, S: float, dchi: float, delta: float, RDPhaseFlag: int
) -> float:
    """
...
def IMRPhenomX_Ringdown_Phase_22_d12(
    eta: float, S: float, dchi: float, delta: float, RDPhaseFlag: int
) -> float:
    """
    ...
    if RDPhaseFlag == 105:
        noSpin = (
            eta
            * (0.7207992174994245 - 1.237332073800276 * eta + 6.086871214811216 * eta2)
        ) / (
        ...
        eqSpin = (
            ...
            * (
                ...
                + 835.1725103648205 * eta3
            )
            * S2
            + (
                0.20035146870472367
                - 0.28745205203100666 * eta
                - 47.56042058800358 * eta4
            )
            * S3
            + eta
            * (5.7756520242745735 - 43.97332874253772 * eta + 338.7263666984089 * eta3)
            * S4
            + (
                -0.2697933899920511
                + 4.917070939324979 * eta
                - 22.384949087140086 * eta4
...
def IMRPhenomX_Ringdown_Phase_22_d24(
    eta: float, S: float, dchi: float, delta: float, RDPhaseFlag: int
) -> float:
    """
    ...
    if RDPhaseFlag == 105:
        noSpin = (
            eta
            * (-9.460253118496386 + 9.429314399633007 * eta + 64.69109972468395 * eta2)
        ...
        eqSpin = (
            ...
            + eta4
            * S
            * (-774.3633787391745 - 2177.554979351284 * S - 1031.846477275069 * S2)
            + eta2
            * S
            * (-191.00932194869588 - 62.997389062600035 * S + 64.42947340363101 * S2)
...
def IMRPhenomX_Ringdown_Phase_22_d34(
    eta: float, S: float, dchi: float, delta: float, RDPhaseFlag: int
) -> float:
    """
    ...
    if RDPhaseFlag == 105:
        ...
        eqSpin = (
            eta
            * (
                1.7280582989361533 * S
                + 18.41570325463385 * S3
                - 13.743271480938104 * S4
            )
            + eta2
            * (73.8367329022058 * S - 95.57802408341716 * S3 + 215.78111099820157 * S4)
...
def IMRPhenomX_Ringdown_Phase_22_d54(
    eta: float, S: float, dchi: float, delta: float, RDPhaseFlag: int
) -> float:
    """
    ...
    if RDPhaseFlag == 105:
        noSpin = (
            eta
            * (7.05731400277692 + 22.455288821807095 * eta + 119.43820622871043 * eta2)
...
def IMRPhenomX_Ringdown_Phase_22_Ansatz(
    ...
    if RDPhaseFlag == 105:
        ...
        phaseRD = (
            ...
            + pPhase["c2"] * invf2
            + pPhase["c4"] * invf4
            + (pPhase["cL"] / (fda * fda + (ff - frd) * (ff - frd)))
        )
...
def IMRPhenomX_Ringdown_Phase_22_AnsatzInt(
    ff: float, powers_of_f: "IMRPhenomX_UsefulPowers", pWF: dict, pPhase: dict
) -> float:
    """
