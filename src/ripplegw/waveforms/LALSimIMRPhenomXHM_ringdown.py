    ...
    if RDAmpFlag == 122018:
        ...
        eqSpin = (
            ...
            * (
                ...
                - 0.0013927779195756036 * S
                + eta2
                * (
                    -0.054621206928483663
                    + 0.025956604949552205 * S
                    + 0.020360826886107204 * S2
                )
            )
        )

        uneqSpin = (
            -0.018115657394753674
            * pWF["dchi"]
            * eta2
            * (1.0 - 10.539795474715346 * eta2 * delta)
        )
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_21_alambda: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_21_lambda(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        noSpin = (
            0.5566284518926176 + 0.12651770333481904 * eta + 1.8084545267208734 * eta2
        )

        eqSpin = (
            0.29074922226651545
            + eta2 * (-2.101111399437034 - 3.4969956644617946 * S)
            + eta * (0.059317243606471406 - 0.31924748117518226 * S)
            + 0.27420263462336675 * S
        ) * S
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_21_lambda: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_21_sigma(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_21_sigma: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_21_rdcp1(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_21_rdcp1: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_21_rdcp2(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_21_rdcp2: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_21_rdcp3(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_21_rdcp3: version {RDAmpFlag} is not valid."
        )
    ...
    if RDAmpFlag == 122018:
        ...
        eqSpin = (
            ...
            * (
                ...
                + eta * (-0.013132320758494439 - 0.018186317026076343 * S)
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_33_alambda: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_33_lambda(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        noSpin = (
            0.7435306475478924 - 0.06688558533374556 * eta + 1.471989765837694 * eta2
        )

        eqSpin = S * (
            0.19457194111990656
            + 0.07564220573555203 * S
            + eta
            * (-0.4809350398289311 + 0.17261430318577403 * S - 0.1988991467974821 * S2)
        )
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_33_lambda: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_33_sigma(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_33_sigma: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_33_rdcp1(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_33_rdcp1: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_33_rdcp2(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_33_rdcp2: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_33_rdcp3(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_33_rdcp3: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_32_alambda(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        eqSpin = S * (
            ...
            + eta4 * (-0.7849112300598181 - 2.09188976953315 * S)
            + eta2 * (0.08311497969032984 - 0.15569578955822236 * S)
            + eta * (-0.01083175709906557 + 0.00568899459837252 * S)
            - 0.00009363591928190229 * S
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_32_alambda: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_32_lambda(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        noSpin = (
            jnp.sqrt(1.0 - 3.0 * eta)
            * (
                0.0341611244787871
                - 0.3197209728114808 * eta
        ...
        eqSpin = (
            ...
            * (
                0.11057199932233873
                + eta2 * (25.536336676250748 - 71.18182757443142 * S)
                + 9.790509295728649 * eta * S
                + eta3 * (-56.96407763839491 + 175.47259563543165 * S)
            )
        )
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_32_lambda: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_32_sigma(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_32_sigma: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_32_rdaux1(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_32_rdaux1: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_32_rdaux2(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_32_rdaux2: version {RDAmpFlag} is not valid."
        )
    ...
    if RDAmpFlag == 122018:
        ...
        eqSpin = (
            ...
            * (
                ...
                + 0.004122811292229324 * S
                + eta * (-0.06166686913913691 + 0.014107365722576927 * S) * S
                + eta2
                * (
                    -0.2945455034809188
                    + 0.4139026619690879 * S
                    - 0.1389170612199015 * S2
                )
                + eta3
                * (
                    0.9225758392294605
                    - 0.9656098473922222 * S
                    + 0.19708289555425246 * S2
                )
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_44_a: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_44_lambda(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        eqSpin = (
            -0.03011582009308575 * S
            + 0.09034746027925727 * eta * S
            + 1.8738784391649446 * eta2 * S
            - 5.621635317494836 * eta3 * S
        ) / (-1.1340218677260014 + S)

        uneqSpin = (
            0.959943270591552 * pWF["dchi"] ** 2 * eta2
            + 0.853573071529436 * pWF["dchi"] * eta2 * delta
        )
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_44_lambda: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_44_sigma(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122018:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_44_sigma: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_44_rdcp1(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_44_rdcp1: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_44_rdcp2(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_44_rdcp2: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_44_rdcp3(pWF: dict, RDAmpFlag: int) -> float:
    ...
    if RDAmpFlag == 122022:
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Amp_44_rdcp3: version {RDAmpFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Phase_32_p1(pWF: dict, RDPhaseFlag: int) -> float:
    ...
    if RDPhaseFlag == 122019:
        ...
        uneqSpin = (
            ...
            * (
                641.2473192044652
                - 1600.240100295189 * pWF["chi1L"] * eta
                + 1600.240100295189 * pWF["chi2L"] * eta
        ...
        total = (
            3221.932636435056
            - 134.59521937837278 * eta1
            + chidiff1
            * delta
            * (
                319.06177738567885 * eta2
                - 20578.40007594641 * eta3
            ...
            * (
                24.059115241154707
                * (
                    436.41245673494626 * eta2
                    - 2938.503437122471 * eta3
                    + 5027.414440730744 * eta4
                )
                + 21.848741292340005
                * (
                    1251.706577839354 * eta2
                    - 14171.490147583942 * eta3
                    + 36914.6553449061 * eta4
                )
                * S1
                + 15.901300902033508
                * (
                    -149.2789474539545 * eta2
                    + 3483.608736789833 * eta3
                    - 11289.97178789606 * eta4
                )
                * S2
            )
            / (-1.803337035190313 + S1)
            - 0.012868288384497346
            / (0.000187943994873322 + (-0.1923690355128322 + eta1) ** 2)
        )

    else:
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Phase_32_p1: version {RDPhaseFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Phase_32_p2(pWF: dict, RDPhaseFlag: int) -> float:
    ...
    if RDPhaseFlag == 122019:
        ...
        uneqSpin = 477.51566939885424 * pWF["dchi"] * jnp.sqrt(1.0 - 4.0 * eta) * eta2
        ...
        total = (
            3169.5772463611165
            + 56.56534589293562 * eta1
            - 863.5731390762933 * eta2
            + 2693.8619211321557 * eta3
            + chidiff1
            * delta
            * (
                -2818.2944800258847 * eta2
                + 15684.658457287562 * eta3
            ...
            * (
                -0.16388886708177886
                * (
                    334.30009385854424 * eta2
                    - 154749.10305716714 * eta3
                    + 613903.6107269318 * eta4
                )
                + 11.950465013745157
                * (
                    1079.481585746054 * eta2
                    - 11981.85336876442 * eta3
                    + 30911.708103120814 * eta4
                )
                * S1
            )
            / (-1.169876031327984 + S1)
            - 0.009425837438775205
            / (0.00016960223009674388 + (-0.20083535429185695 + eta1) ** 2)
        )

    else:
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Phase_32_p2: version {RDPhaseFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Phase_32_p3(pWF: dict, RDPhaseFlag: int) -> float:
    ...
    if RDPhaseFlag == 122019:
        ...
        uneqSpin = 378.2359918274837 * pWF["dchi"] * jnp.sqrt(1.0 - 4.0 * eta) * eta2
        ...
        total = (
            3119.6603946770488
            + 168.61554447853712 * eta1
            - 1777.6654596491376 * eta2
            + 5037.407962552042 * eta3
            + chidiff1
            * delta
            * (
                45693.135566736484 * eta2
                - 789332.4959926775 * eta3
                + 4.460496312695218e6 * eta4
                - 8.176309211912101e6 * eta5
            )
            + chidiff1
            * delta
            * (
                7840.121424232572 * eta2
                - 47166.09840761356 * eta3
                + 66597.52917033392 * eta4
            )
            * S1
            + S1
            * (
                -6.019579546899472
                * (
                    14538.163921822728 * eta2
                    - 318911.4362763759 * eta3
                    + 2.6041867832020866e6 * eta4
                    - 9.288489508236282e6 * eta5
                    + 1.2170972980338342e7 * eta6
                )
                - 7.739304888898913
                * (
                    11114.219992659659 * eta2
                    - 231541.9569739445 * eta3
                    + 1.8069370995120746e6 * eta4
                    - 6.203273456127891e6 * eta5
                    + 7.874294046591697e6 * eta6
                )
                * S1
            )
            - 0.0003538235766427527
            / (8.2810517855422e-6 + (-0.2075868299995718 + eta1) ** 2)
        )

    else:
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Phase_32_p3: version {RDPhaseFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Phase_32_p4(pWF: dict, RDPhaseFlag: int) -> float:
    ...
    if RDPhaseFlag == 122019:
        ...
        noSpin = (
            3077.0657367004565 + 64.99844502520415 * eta - 357.38692756785395 * eta2
        )
        ...
        uneqSpin = 390.5443469721231 * pWF["dchi"] * jnp.sqrt(1.0 - 4.0 * eta) * eta2
        ...
        total = (
            3063.2702533356364
            + 74.20321762511647 * eta1
            - 346.6653326379183 * eta2
            + chidiff1
            * delta
            * (
                2604.6711121030685 * eta2
                - 25322.83641432119 * eta3
                + 64521.907625802785 * eta4
            )
            + 17.748987975469845
            * (
                6659.676835974436 * eta2
                - 207712.54687648916 * eta3
                + 2.5247192995989644e6 * eta4
                - 1.4825576629165621e7 * eta5
                + 4.215660954626601e7 * eta6
                - 4.662796037240443e7 * eta7
            )
            * S1
            / (-1.2617538728082525 + S1)
            - 2.211595095940034e-6
            / (-0.000010240443266599763 + (-0.17594819839300638 + eta1) ** 2)
        )

    else:
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Phase_32_p4: version {RDPhaseFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Phase_32_p5(pWF: dict, RDPhaseFlag: int) -> float:
    ...
    if RDPhaseFlag == 122022:
        ...
        total = (
            3099.009925231132
            - 5.823302919769178 * eta1
            + chidiff1
            * delta
            * (
                1936.2628510750844 * eta2
                - 32280.62038532877 * eta3
                + 97943.49145078743 * eta4
            )
            + 2.8281136508769498
            * (
                6856.762845446495 * eta2
                - 201653.94475043533 * eta3
                + 2.4217205751584964e6 * eta4
                - 1.4582166806161262e7 * eta5
                + 4.377208838391116e7 * eta6
                - 5.214533274483399e7 * eta7
            )
            * S1
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Phase_32_p5: version {RDPhaseFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Phase_Ansatz(
    ...
    if pWFHM["MixingOn"] == 0:
        ...
        if pWFHM["IMRPhenomXHMRingdownPhaseVersion"] == 122019:
            ...
            if pWFHM["fPhaseRDflat"] > 0 and IMRPhenomX_StepFuncBool(
                ff, pWFHM["fPhaseRDflat"]
            ):
                dphaseRD = (
                    pPhase["RDCoefficient"][5]
                    + pPhase["RDCoefficient"][6] * powers_of_f["m_five"]
                )
            else:
                # ansatz: a0 + a1/f + a2/f^2 + a3/f^4 + a4*fdamplm/(fdamplm^2 + (f - fRDlm)^2)
                dphaseRD = (
                    pPhase["RDCoefficient"][0]
                    ...
                    + pPhase["RDCoefficient"][4]
                    * fda
                    / (fda * fda + (ff - frd) * (ff - frd))
                )
    else:
        raise ValueError(
...
def IMRPhenomXHM_RD_Phase_AnsatzInt(
    ff: float, powers_of_f: dict, pWFHM: dict, pPhase: dict
) -> float:
    """
...
def IMRPhenomXHM_RD_Phase_DerAnsatz(
    ff: float, powers_of_f: dict, pWFHM: dict, pPhase: dict
) -> float:
    """
    ...
    if pWFHM["MixingOn"] == 0:
        ...
        ddphaseRD = -2 * frd * frd * pPhase["alpha2"] * powers_of_f[
            "m_three"
        ] - 2 * pPhase["alphaL"] * fda * (ff - frd) / jnp.power(
            fda * fda + (ff - frd) * (ff - frd), 2
        )
        ...
        if pWFHM["IMRPhenomXHMRingdownPhaseVersion"] == 122019:
            ...
            ddphaseRD = (
                -2 * pPhase["alpha2_S"] * powers_of_f["m_three"]
                - 4 * pPhase["alpha4_S"] * powers_of_f["m_five"]
                - 2
                * pPhase["alphaL_S"]
                * fda
                * (ff - frd)
                / jnp.power(fda * fda + (ff - frd) * (ff - frd), 2)
            )
        else:
            # d/df of: a0 + a1/f + a2/f^2 + a3/f^4 + a4*fdamplm/(fdamplm^2 + (f - fRDlm)^2)
            ddphaseRD = (
                -pPhase["RDCoefficient"][1] * powers_of_f["m_two"]
...
def IMRPhenomXHM_RD_Phase_32_SpheroidalTimeShift(pWF: dict, RDPhaseFlag: int) -> float:
    ...
    if RDPhaseFlag == 122019:
        ...
        uneqSpin = 465.7904934097202 * pWF["dchi"] * jnp.sqrt(1.0 - 4.0 * eta) * eta2
        ...
        total = (
            ...
            * (
                18.280316689743625
                * (
                    46.16185108285708 * eta1
                    - 515.3588338752849 * eta2
                    + 1475.462268010926 * eta3
                )
                + 2.1970246269696427
                * (
                    370.70040479593024 * eta1
                    - 4329.153306191607 * eta2
                    + 12065.631680744644 * eta3
                )
                * S1
            )
            + (
                12.080898026205173
                - 79.10914761468462 * eta1
                - 89.82426456495799 * eta2
                + 915.6864093792078 * eta3
            )
    ...
    if RDPhaseFlag == 122019:
        ...
        uneqSpin = (
            ...
            * (
                pWF["chi2L"]
                * jnp.power(eta, 2.5)
                * (88.56162028006072 - 30.01812659282717 * S)
        ...
        total = (
            ...
            * delta
            * (
                -4055.661463620154 * eta3
                + 48053.79215999518 * eta4
                - 126644.96534635697 * eta5
            )
            + chidiff2
            * (
                1489.2939046368886 * eta3
            ...
            * (
                ...
                * (
                    ...
                    - 110112.73697613904 * eta4
                )
                * S2
            )
            + (
                -1.3903824533899325
                + 13.761709564309667 * eta1
                - 10.633427224975128 * eta2
                - 263.01839936998965 * eta3
...
def IMRPhenomXHM_RD_Phase_22_alpha2(pWF: dict, RDPhaseFlag: int) -> float:
    ...
    if RDPhaseFlag == 122019:
        ...
        uneqSpin = (
            pWF["dchi"]
            * (
                0.9088578269496244 * eta**2.5
                + 15.619592332008951 * pWF["dchi"] * eta**3.5
            )
            * delta
        )
        ...
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Phase_22_alpha2: version {RDPhaseFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Phase_22_alphaL(pWF: dict, RDPhaseFlag: int) -> float:
    ...
    if RDPhaseFlag == 122019:
        ...
        uneqSpin = (
            -0.7428134042821221 * pWF["chi1L"] * eta**3.5
            + 0.7428134042821221 * pWF["chi2L"] * eta**3.5
            + 17.588573345324154 * pWF["chi1L"] ** 2 * eta**4.5
            - 35.17714669064831 * pWF["chi1L"] * pWF["chi2L"] * eta**4.5
            + 17.588573345324154 * pWF["chi2L"] ** 2 * eta**4.5
        ) * delta

        total = noSpin + eqSpin + uneqSpin
    else:
        raise ValueError(
            f"Error in IMRPhenomXHM_RD_Phase_22_alphaL: version {RDPhaseFlag} is not valid."
        )
...
def IMRPhenomXHM_RD_Amp_Ansatz(powers_of_Mf: dict, pWFHM: dict, pAmp: dict) -> float:
    ...
    if RDAmpFlag == 0:
        ...
        ampRD = (
            (fda * jnp.abs(pAmp["RDCoefficient"][0]) * pAmp["RDCoefficient"][2])
        ...
        if pAmp["nCoefficientsRDAux"] > 0 and not IMRPhenomX_StepFuncBool(
            ff, pAmp["fRDAux"]
        ):
            # Polynomial
            fpower = 1.0
        ...
        # Check if we should use exponential falloff
        elif pWFHM["fAmpRDfalloff"] > 0 and IMRPhenomX_StepFuncBool(
            ff, pWFHM["fAmpRDfalloff"]
        ):
            ampRD = pAmp["RDCoefficient"][3] * jnp.exp(
                -pAmp["RDCoefficient"][4] * (ff - pWFHM["fAmpRDfalloff"])
            )
        else:
            ...
            ampRD = (
                ...
                * fda
                / (
                    jnp.exp(pAmp["RDCoefficient"][1] / dfd * dfr)
                    * (dfr * dfr + dfd * dfd)
                )
            )
...
def IMRPhenomXHM_RD_Amp_DAnsatz(powers_of_Mf: dict, pWF: dict, pAmp: dict) -> float:
    ...
    if RDAmpFlag == 0:
        ...
        numerator = (
            jnp.abs(pAmp["RDCoefficient"][0])
            * jnp.power(ff, -1.0 - lc)
            * (
                dfd * lc * (frd * frd + dfd * dfd)
                + ff
                * (
                    frd * frd * lambda_coeff
                    - 2.0 * dfd * frd * (1.0 + lc)
                    + dfd * dfd * lambda_coeff
                )
        ...
        denom = (
            dfr * dfr + fda * fda * pAmp["RDCoefficient"][2] * pAmp["RDCoefficient"][2]
        )
        denom = (
            pAmp["RDCoefficient"][2]
            * denom
...
def IMRPhenomXHM_RD_Amp_NDAnsatz(
    ...
    fun2R = jnp.abs(
        SpheroidalToSpherical(
            powers_of_fun, pAmp22, pPhase22, pAmp, pPhase, pWFHM, pWF22
        )
    )
...
def IMRPhenomXHM_Ringdown_Amplitude_Veto(
    V2: float, V3: float, V4: float, pWFHM: dict, pWF22: dict
) -> tuple:
    """
...
def IMRPhenomXHM_RD_Amp_Coefficients(pWF22: dict, pWFHM: dict, pAmp: dict) -> dict:
    ...
    if RDAmpVersion == 0:
        ...
        pAmp_out["RDCoefficient"][1] = pAmp["RingdownAmpFits"][
            pWFHM["modeInt"] * 3 + 1
        ](pWF22, pWFHM["IMRPhenomXHMRingdownAmpFitsVersion"])
        pAmp_out["RDCoefficient"][2] = pAmp["RingdownAmpFits"][
            pWFHM["modeInt"] * 3 + 2
        ](pWF22, pWFHM["IMRPhenomXHMRingdownAmpFitsVersion"])
        ...
        if RDAmpVersion == 1:
            ...
            pAmp_out["RDCoefficient"][1] = (
                0.5 * pAmp_out["RDCoefficient"][2] * jnp.log(rdcp1 / rdcp3)
            )
        ...
        if pWFHM.get("fAmpRDfalloff", 0) > 0:
            ...
            pAmp_out["RDCoefficient"][4] = (
                -1.0
                * IMRPhenomXHM_RD_Amp_DAnsatz(powers_of_RDfalloff, pWFHM_temp, pAmp_out)
...
def IMRPhenomXHM_RDAux_Amp_Coefficients(pWF22: dict, pWFHM: dict, pAmp: dict) -> dict:
    ...
    for i in range(nCollocPts):
        val = jnp.abs(
            pAmp["RingdownAmpFits"][24 + i](
                pWF22, pWFHM["IMRPhenomXHMRingdownAmpFitsVersion"]
            )
        )
        values_list.append(val)

    CollocationPointsValuesAmplitudeRDAux = CollocationPointsValuesAmplitudeRDAux.at[
        :nCollocPts
    ].set(jnp.array(values_list))
    ...
    if pWFHM.get("RingdownAmpVeto", 0) == 2:
        if (
            CollocationPointsValuesAmplitudeRDAux[nCollocPts - 1]
            < CollocationPointsValuesAmplitudeRDAux[nCollocPts]
        ):
            CollocationPointsValuesAmplitudeRDAux = (
                CollocationPointsValuesAmplitudeRDAux.at[nCollocPts - 1].set(
                    CollocationPointsValuesAmplitudeRDAux[nCollocPts]
                )
            )
    ...
    CollocationPointsFreqsAmplitudeRDAux = CollocationPointsFreqsAmplitudeRDAux.at[
        0
    ].set(fAmpMatchIM)
    CollocationPointsFreqsAmplitudeRDAux = CollocationPointsFreqsAmplitudeRDAux.at[
        1
    ...
    pAmp_out["CollocationPointsFreqsAmplitudeRDAux"] = (
        CollocationPointsFreqsAmplitudeRDAux.tolist()
    )
    pAmp_out["CollocationPointsValuesAmplitudeRDAux"] = (
        CollocationPointsValuesAmplitudeRDAux.tolist()
    )
