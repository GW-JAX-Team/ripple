...
def IMRPhenomX_Intermediate_Amp_22_v2(
    eta: float, STotR: float, dchi: float, delta: float, version: int
) -> float:
    """
    ...
    noSpin = (
        2.2436523786378983
        + 2162.4749081764216 * eta
        + 24460.158604784723 * eta2
        - 12112.140570900956 * eta3
    ) / (1.0 + 120.78623282522702 * eta + 416.4179522274108 * eta2)

    eqSpin = (
        STotR
        * (
            6.727511603827924
            + eta2 * (414.1400701039126 - 234.3754066885935 * STotR)
            - 5.399284768639545 * STotR
            + eta * (-186.87972530996245 + 128.7402290554767 * STotR)
        )
    ) / (3.24359204029217 - 3.975650468231452 * STotR + 1.0 * S2)
...
def IMRPhenomX_Intermediate_Amp_22_v3(
    eta: float, STotR: float, dchi: float, delta: float, version: int
) -> float:
    """
    ...
    noSpin = (
        1.195392410912163
        + 1677.2558976605421 * eta
        + 24838.37133975971 * eta2
        - 17277.938868280915 * eta3
    ) / (1.0 + 144.78606839716073 * eta + 428.8155916011666 * eta2)

    eqSpin = (
        STotR
        * (
            -2.1413952025647793
            + 0.5719137940424858 * STotR
            + eta
            * (
                46.61350006858767
                + 0.40917927503842105 * STotR
                - 11.526500209146906 * S2
            )
            + 1.1833965566688387 * S2
            + eta2
            * (-84.82318288272965 - 34.90591158988979 * STotR + 19.494962340530186 * S2)
        )
    ) / (-1.4786392693666195 + 1.0 * STotR)
...
def IMRPhenomX_Intermediate_Amp_22_vA(
    eta: float, STotR: float, dchi: float, delta: float, version: int
) -> float:
    """
    ...
    eqSpin = (
        ...
        * (
            ...
            - 19.38970173389662 * S2
            + eta3
            * (1612.2681322644232 - 6962.675551371755 * STotR + 1486.4658089990298 * S2)
        )
    ) / (12.647425554323242 - 10.540154508599963 * STotR + 1.0 * S2)
...
def IMRPhenomX_Intermediate_Amp_22_delta0(
    d1: float,
    d4: float,
    V1: float,
    V2: float,
    ...
    if version == 1043:  # no left derivative; 1043 is used in IMRPhenomXHM
        ...
        raise ValueError(
            f"Error in IMRPhenomX_Intermediate_Amp_22_delta0: version {version} is not valid. Recommended flag is 104."
        )

    return retVal


def IMRPhenomX_Intermediate_Amp_22_delta1(
    d1: float,
    d4: float,
    V1: float,
    V2: float,
    ...
    if version == 1043:  # no left derivative; 1043 is used in IMRPhenomXHM
        ...
        raise ValueError(
            f"Error in IMRPhenomX_Intermediate_Amp_22_delta1: version {version} is not valid. Recommended flag is 104."
        )

    return retVal


def IMRPhenomX_Intermediate_Amp_22_delta2(
    d1: float,
    d4: float,
    V1: float,
    V2: float,
    ...
    if (
        version == 1043
    ):  # no left derivative: v1, v2, v3, v4, d4; 1043 is used in IMRPhenomXHM
        f12 = f1 * f1
        f13 = f12 * f1
        f14 = f13 * f1
        ...
        raise ValueError(
            f"Error in IMRPhenomX_Intermediate_Amp_22_delta2: version {version} is not valid. Recommended flag is 104."
        )

    return retVal


def IMRPhenomX_Intermediate_Amp_22_delta3(
    d1: float,
    d4: float,
    V1: float,
    V2: float,
    ...
    if (
        ...
        raise ValueError(
            f"Error in IMRPhenomX_Intermediate_Amp_22_delta3: version {version} is not valid. Recommended flag is 104."
        )

    return retVal


def IMRPhenomX_Intermediate_Amp_22_delta4(
    d1: float,
    d4: float,
    V1: float,
    V2: float,
    ...
    if (
        ...
        raise ValueError(
            f"Error in IMRPhenomX_Intermediate_Amp_22_delta4: version {version} is not valid. Recommended flag is 104."
        )

    return retVal


def IMRPhenomX_Intermediate_Amp_22_delta5(
    d1: float,
    d4: float,
    V1: float,
    V2: float,
    ...
    if (
        ...
        raise ValueError(
            f"Error in IMRPhenomX_Intermediate_Amp_22_delta5: version {version} is not valid. Recommended flag is 104."
        )
...
def IMRPhenomX_Intermediate_Phase_22_Ansatz(
    ff: float, powers_of_f: "IMRPhenomX_UsefulPowers", pWF: dict, pPhase: dict
) -> float:
    """
...
def IMRPhenomX_Intermediate_Phase_22_AnsatzInt(
    f: float, powers_of_f: "IMRPhenomX_UsefulPowers", pWF: dict, pPhase: dict
) -> float:
    """
