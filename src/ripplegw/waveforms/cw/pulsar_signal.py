...
from ripplegw.constants import PI, TWO_PI
from ripplegw.typing import FloatLike
from ripplegw.waveforms.cw.barycenter import emission_delay, roemer_delay
from ripplegw.waveforms.cw.earth import earth_state


def _phase_taylor(
    tau: Float[Array, " n"], f0: FloatLike, fkdot: Float[Array, " k"] | tuple
) -> Float[Array, " n"]:
    """Wave phase ``2π Σ f^{(s)}/(s+1)! · τ^{s+1}`` (excluding φ₀).

    Args:
        tau (Float[Array, " n"]): SSB time relative to the reference epoch (s).
        f0 (FloatLike): Wave frequency at the reference epoch (Hz).
...
def exact_pulsar_polarizations(
    ...
    Args:
        ...
        f0 (FloatLike): Wave frequency at the reference epoch (Hz).
        phi0 (FloatLike): Initial phase ``φ₀`` at the reference epoch (radians).
        aplus (FloatLike): Plus polarization amplitude ``A₊``.
        across (FloatLike): Cross polarization amplitude ``A_×``.
...
def generate_pulsar_polarizations(
    ...
    Args:
        ...
        alpha, delta (FloatLike): Source sky position (radians).
        f0 (FloatLike): Wave frequency at the reference epoch (Hz).
        phi0 (FloatLike): Initial phase at the reference epoch (radians).
        aplus, across (FloatLike): Polarization amplitudes.
        det_location_m (tuple): Detector geocentric location (metres).
...
def generate_binary_pulsar_polarizations(
    ...
    Args:
        ...
        asini (FloatLike): Projected semi-major axis ``a sin i`` (light-seconds).
        ecc (FloatLike): Orbital eccentricity.
        period (FloatLike): Orbital period (seconds).
        argp (FloatLike): Argument of periapsis ``ω`` (radians).
