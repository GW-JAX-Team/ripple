...
from ..constants import G, C, MTSUN, PI
import jax
...
from .LALSimIMRPhenomX_PNR_coefficients import IMRPhenomX_PNR_beta_Bf_coefficient
...
class IMRPhenomXGetAndSetPrecessionVariables:
    ...
    J0x_Sf: float = field(init=False, default=0.0)
    J0y_Sf: float = field(init=False, default=0.0)
    J0z_Sf: float = field(init=False, default=0.0)
    J0: float = field(init=False, default=0.0)
    thetaJ_Sf: float = field(init=False, default=0.0)
    phiJ_Sf: float = field(init=False, default=0.0)
    Nx_Sf: float = field(init=False, default=0.0)
    Ny_Sf: float = field(init=False, default=0.0)
    Nz_Sf: float = field(init=False, default=0.0)
    Xx_Sf: float = field(init=False, default=0.0)
    Xy_Sf: float = field(init=False, default=0.0)
    Xz_Sf: float = field(init=False, default=0.0)
    kappa: float = field(init=False, default=0.0)

    # Orbital angular momentum
    L0: float = field(init=False, default=0.0)
    L1: float = field(init=False, default=0.0)
    L2: float = field(init=False, default=0.0)
    L3: float = field(init=False, default=0.0)
    L4: float = field(init=False, default=0.0)
    L5: float = field(init=False, default=0.0)
    L6: float = field(init=False, default=0.0)
    L7: float = field(init=False, default=0.0)
    L8: float = field(init=False, default=0.0)
    L8L: float = field(init=False, default=0.0)
    ...
    alpha_spline: Interpolator1D = field(init=False)
    cosbeta_spline: Interpolator1D = field(init=False)
    gamma_spline: Interpolator1D = field(init=False)

    Mfmax_angles: float = field(init=False, default=0.0)
    alpha_ref: float = field(
        init=False, default=0.0
    )  # Record value of alpha at f_ref */
    gamma_ref: float = field(
        init=False, default=0.0
    )  # Record value of gamma at f_ref */
    alpha_ftrans: float = field(
        init=False, default=0.0
    )  # Record value of alpha at end of inspiral, used in IMRPhenomXPHMTwistUp and IMRPhenomXPHMTwistUpOneMode */
    cosbeta_ftrans: float = field(
        init=False, default=0.0
    )  # Record value of cosbeta at end of inspiral, used in IMRPhenomXPHMTwistUp and IMRPhenomXPHMTwistUpOneMode */
    gamma_ftrans: float = field(
        init=False, default=0.0
    )  # Record value of gamma at end of inspiral, used in IMRPhenomXPHMTwistUp and IMRPhenomXPHMTwistUpOneMode */
    gamma_in: float = field(
        init=False, default=0.0
    )  # Record last value of gamma, used in IMRPhenomXPHMTwistUp and IMRPhenomXPHMTwistUpOneMode */
    alpha_params: dict = (
        field(  # Parameters needed for analytical MRD continuation of alpha */
            init=False, default_factory=lambda: dict(aRD=0.0, bRD=0.0, cRD=0.0)
        )
    )
    beta_params: dict = (
        field(  # Parameters needed for analytical MRD continuation of beta */
            init=False,
    ...
    ftrans_MRD: float = field(init=False, default=0.0)
    fmax_inspiral: float = field(init=False, default=0.0)
    # precessing_tag
    deltaf_interpolation: float = field(init=False, default=0.0)
    M_MIN: int = field(init=False, default=2)
    M_MAX: int = field(init=False, default=2)
    L_MAX_PNR: int = field(init=False, default=0)
    integration_buffer: float = field(
        init=False, default=0.0
    )  # Buffer region for integration of SpinTaylor equations: added so that interpolated angles cover the frequency range requested by user  */
    fmin_integration: float = field(
        init=False, default=0.0
    )  # Minimum frequency covered by the integration of PN spin-precessing equations for SpinTaylor models  */
    Mfmin_integration: float = field(
        init=False, default=0.0
    )  # Minimum frequency covered by the integration of PN spin-precessing equations for SpinTaylor models  */

    # PNR-specific additions for single-spin mapping
    chi_singleSpin: float = field(init=False, default=0.0)
    costheta_singleSpin: float = field(init=False, default=0.0)
    costheta_final_singleSpin: float = field(init=False, default=0.0)

    theta_antisymmetric: float = field(init=False, default=0.0)

    PNR_HM_Mflow: float = field(init=False, default=0.0)
    PNR_HM_Mfhigh: float = field(init=False, default=0.0)
    PNR_q_window_lower: float = field(
        init=False, default=0.0
    )  # Boundary values for PNR angle transition window
    PNR_q_window_upper: float = field(init=False, default=0.0)
    PNR_chi_window_lower: float = field(init=False, default=0.0)
    PNR_chi_window_upper: float = field(init=False, default=0.0)
    PNRInspiralScaling: int = field(
        init=False, default=0
    )  # Enforce inspiral scaling for HM angles outside of calibration window

    IMRPhenomXPNRInterpTolerance: float = field(
        init=False, default=0.01
    )  # Tolerance for HM angle interpolation
    ...
    def __post_init__(self):
        ...
        # prec_family_idx = self.IMRPhenomXPrecVersion // 100  # 1: NNLO; 2: MSA; 3: SpinTaylor (numerical)
        # # Since these methods mutate self, we need to call them conditionally
        # if prec_family_idx == 2:
        #     self.compute_evolved_spin_using_msa()
        # elif prec_family_idx == 3:
        #     self.compute_evolved_spin_using_spintaylor()
        # else: NNLO case, not implemented yet
    ...
    def _compute_spin_quantities(self):
        ...
        object.__setattr__(
            ...
            "chi1_norm",
            jnp.sqrt(
                self.chi1x * self.chi1x
                + self.chi1y * self.chi1y
                + self.chi1z * self.chi1z
            ),
        )
        object.__setattr__(
            self,
            "chi2_norm",
            jnp.sqrt(
                self.chi2x * self.chi2x
                + self.chi2y * self.chi2y
                + self.chi2z * self.chi2z
            ),
        )
        ...
        object.__setattr__(
            self,
            "chi1_perp",
            jnp.sqrt(self.chi1x * self.chi1x + self.chi1y * self.chi1y),
        )
        object.__setattr__(
            self,
            "chi2_perp",
            jnp.sqrt(self.chi2x * self.chi2x + self.chi2y * self.chi2y),
        )
        ...
        object.__setattr__(
            self,
            "S1_perp",
            self.m1_2 * jnp.sqrt(self.chi1x * self.chi1x + self.chi1y * self.chi1y),
        )
        object.__setattr__(
            self,
            "S2_perp",
            self.m2_2 * jnp.sqrt(self.chi2x * self.chi2x + self.chi2y * self.chi2y),
        )
        ...
        object.__setattr__(
            ...
            "STot_perp",
            jnp.sqrt(
                (self.S1x + self.S2x) * (self.S1x + self.S2x)
                + (self.S1y + self.S2y) * (self.S1y + self.S2y)
            ),
        )
    ...
    def _setup_version_flags(self):
        ...
        version = jnp.where(
            self.IMRPhenomXPrecVersion == 300, 223, self.IMRPhenomXPrecVersion
        )
        ...
        cond = (
            (chi_in_plane < 1e-7)
            & (self.lalParams["PNRUseTunedAngles"] == 1)
            & (self.pWF["PNR_SINGLE_SPIN"] != 1)
        )
        self.lalParams["PNRUseTunedAngles"] = jnp.where(
            cond, False, self.lalParams["PNRUseTunedAngles"]
        )
        self.lalParams["AntisymmetricWaveform"] = jnp.where(
            cond, False, self.lalParams["AntisymmetricWaveform"]
        )
        self.lalParams["PNRUseTunedCoprec"] = jnp.where(
            cond, False, self.lalParams["PNRUseTunedCoprec"]
        )
    ...
    def _setup_spintaylor_prescription(self):
        ...
        integration_buffer, flow = self._select_integration_path(
            integration_buffer_path1, integration_buffer_path2, flow_path1, flow_path2
        )
    ...
    def _initialize_mode_arrays_and_frequencies(self):
        ...
        # Set LMAX_PNR and M_MAX for 22 mode for now
        # TODO: Generalize for other modes
        ...
        object.__setattr__(self, "L_MAX_PNR", 2)
        object.__setattr__(self, "M_MAX", 2)
    ...
    def _compute_path1_parameters(self):
        ...
        integration_buffer_path1 = jnp.where(
            self.pWF["deltaF"] > 0.0, 3.0 * self.pWF["deltaF"], 0.5
        )
    ...
    def _compute_path2_parameters(self):
        ...
        integration_buffer_path2, flow_path2 = self._finalize_path2_parameters(
            flow_path2_intermediate
        )
    ...
    def _compute_path2_initial_frequencies(self):
        ...
        flow_temp_path2 = jnp.where(
            self.pWF["deltaF"] == 0.0, 0.0, iStart_here_path2 * self.pWF["deltaF"]
        )
    ...
    def _compute_frequency_cutoffs(self):
        ...
        IMRPhenomX_PNR_GetAndSetPNRVariables(
            self, self.pWF
        )  ## First precversion is set to 223
        ...
        Mf_beta_lower, Mf_beta_upper = IMRPhenomX_PNR_BetaConnectionFrequencies(
            betaParams
        )
        ...
        MF_high_cut = jnp.where(
            jnp.logical_or(
                MF_high_cut > self.pWF["fCutDef"], MF_high_cut < 0.1 * self.pWF["fRING"]
            ),
            self.pWF["fRING"],
            MF_high_cut,
        )
        ...
        Mf_low_cut = jnp.where(
            jnp.logical_or(Mf_low_cut > self.pWF["fCutDef"], MF_high_cut < Mf_low_cut),
            MF_high_cut / 2.0,
            Mf_low_cut,
        )
    ...
    def _compute_path2_intermediate_flow(
        self, flow_temp_path2, fmin_HM_inspiral, Mf_low_cut, MF_high_cut
    ):
        """Compute intermediate flow frequency for path 2."""
        flow_alpha = XLALSimIMRPhenomXUtilsMftoHz(
            Mf_low_cut * 0.65 * self.M_MAX / 2.0, self.pWF["Mtot"]
        )

        # Compute ringdown frequency adjustment
        Mf_RD_22 = self.pWF["fRING"]
        Mf_RD_lm = IMRPhenomXHM_GenerateRingdownFrequency(
            self.L_MAX_PNR, self.M_MAX, self.pWF
        )

        fmin_HM_ringdown = XLALSimIMRPhenomXUtilsMftoHz(
            XLALSimIMRPhenomXUtilsHztoMf(flow_temp_path2, self.pWF["Mtot"])
            - (Mf_RD_lm - Mf_RD_22),
            self.pWF["Mtot"],
        )
        else_branch_result = jnp.where(
            jnp.logical_and(
                fmin_HM_ringdown < fmin_HM_inspiral, fmin_HM_ringdown > 0.0
            ),
            fmin_HM_ringdown,
            fmin_HM_inspiral,
        )
        ...
        flow_path2_intermediate = jnp.where(
            flow_alpha < flow_temp_path2, fmin_HM_inspiral / 1.5, else_branch_result
        )
    ...
    def _finalize_path2_parameters(self, flow_path2_intermediate):
        ...
        iStart_here_path2_final = jnp.floor(
            flow_path2 / pnr_interpolation_deltaf
        ).astype(int)
        flow_path2 = iStart_here_path2_final * pnr_interpolation_deltaf

        return integration_buffer_path2, flow_path2

    def _select_integration_path(self):
        """Select between path 1 and path 2 based on PNRUseTunedAngles."""
        integration_buffer, flow_path = jax.lax.cond(
            self.lalParams["PNRUseTunedAngles"],
            self._compute_path2_parameters,
            self._compute_path1_parameters,
            operand=None,
        )

        object.__setattr__(self, "integration_buffer", integration_buffer)
        return integration_buffer, flow_path
    ...
    def _get_source_frame_variables(self):
        """
        ...
        Based on lines 765-846 of LALSimIMRPhenomX_precession.c
        """
        L0, L1, L2, L3, L4, L5, L6, L7, L8, L8L = (
            self.flag_222_223_twoPN_non_spinning_orbitan_angular_momentum()
        )
        ...
        tol_condition = (jnp.abs(J0x_Sf) < MAX_TOL_ATAN) & (
            jnp.abs(J0y_Sf) < MAX_TOL_ATAN
        )
        # Compress line 797-825
        # Get azimuthal angle of J0 in the source frame
        phiJ_Sf = get_phiJ_Sf(tol_condition, J0_Sf)

        return J0x_Sf, J0y_Sf, J0z_Sf, J0, thetaJ_Sf, phiJ_Sf, phiRef
    ...
    def flag_222_223_twoPN_non_spinning_orbitan_angular_momentum(self):
        ...
        L3 = (
            ...
            * (
                self.chi1L
                + self.chi2L
                + self.chi1L * self.delta
                - self.chi2L * self.delta
            )
            + 5 * (self.chi1L + self.chi2L) * self.eta
        ) / 6.0
        L4 = (81 + (-57 + self.eta) * self.eta) / 24.0
        L5 = (
            -1650
            * (
                self.chi1L
            ...
            + 1336 * (self.chi1L + self.chi2L) * self.eta
            + 511 * (self.chi1L - self.chi2L) * self.delta * self.eta
            + 28 * (self.chi1L + self.chi2L) * self.eta2
        ) / 600.0
        L6 = (
            10935
            + self.eta
            * (
                ...
                + 1674 * self.eta
                + 7 * self.eta2
                + 2214 * self.common_constants.power_of_lalpi_2
            )
        ) / 1296.0
        L7 = 0.0
        L8 = 0.0
    ...
    def compute_evolved_spin_using_msa(self):
        ...
        self = IMRPhenomX_Initialize_MSA_System(
            self, self.pWF, self.lalParams["ExpansionOrder"]
        )
        # What is the output of this function?

        # TODO if MSA_ERROR: switch to NNLO

        Mfinal, afinal, fRING, fDAMP = IMRPhenomX_SetPrecessingRemnantParams(
            self, self.pWF, self.lalParams
        )
        # The output of this function should be Mfinal, afinal, fring, and fdamp
        # To be checked

        # case 223: Line 691 compute orbital angular momentum
        J0x_Sf, J0y_Sf, J0z_Sf, J0, thetaJ_Sf, phiJ_Sf, phiRef = (
            self._get_source_frame_variables()
        )
        J0_Sf = jnp.array([J0x_Sf, J0y_Sf, J0z_Sf])
        object.__setattr__(self, "J0x_Sf", J0x_Sf)
        object.__setattr__(self, "J0y_Sf", J0y_Sf)
        object.__setattr__(self, "J0z_Sf", J0z_Sf)
        object.__setattr__(self, "J0", J0)
        object.__setattr__(self, "thetaJ_Sf", thetaJ_Sf)
        object.__setattr__(self, "phiJ_Sf", phiJ_Sf)
        ...
        kappa = XLALSimIMRPhenomXatan2tol(
            vout[1], vout[0], self.common_constants.MAX_TOL_ATAN
        )
        ...
        tol_condition = (jnp.abs(vout[0]) < self.common_constants.MAX_TOL_ATAN) & (
            jnp.abs(vout[1]) < self.common_constants.MAX_TOL_ATAN
        )
        ...
        (
            ...
            alpha_offset_4,
            epsilon_offset_4,
        ) = convention_five_or_seven_false(
            self, self.pWF, self.pWF["piM"], self.pWF["fRef"], alpha0, epsilon0
        )
...
def convention_five_or_seven_false(pPrec, pWF, piM, fRef, alpha0, epsilon0):
    ...
    alpha_offset, epsilon_offset = Get_alphaepsilon_atfref(
        pPrec, pWF, mprime, piM, fRef, alpha0, epsilon0
    )
    return (
        alpha_offset,
        epsilon_offset,
...
def Get_alphaepsilon_atfref(pPrec, pWF, mprime, piM, fRef, alpha0, epsilon0):
    ...
    alpha_offset, epsilon_offset = Get_alphaepsilon_atfref_pflag_true(
        pPrec, pWF, omega_ref, alpha0, epsilon0
    )
...
def IMRPhenomX_Return_phi_zeta_costhetaL_MSA(pPrec, pWF, v):
    ...
    cond = jnp.abs(pPrec.Smi2 - pPrec.Spl2) > 1.0e-5
...
def IMRPhenomX_L_norm_3PN_of_v(v: jax.Array, L_norm: float, pPrec) -> float:
    ...
    L_norm3PN = L_norm * (
        1.0 + v2 * (term_0 + v * term_1 + v2 * (term_2 + v * term_3 + v2 * (term_4)))
    )
...
def IMRPhenomX_Return_Roots_MSA(LNorm, JNorm, pPrec):
    ...
    scalar_condition = jnp.logical_or.reduce(
        jnp.array(
            [
                (pPrec.dotS1Ln == 1.0),
                (pPrec.dotS2Ln == 1.0),
    ...
    roots_array = jnp.where(
        jnp.atleast_1d(invalid_case), roots_when_invalid(), roots_when_valid()
    )
...
def IMRPhenomX_psiofv(v, v2, psi0, psi1, psi2, pPrec):
    ...
    return psi0 - 0.75 * pPrec.g0 * pPrec.delta_qq * (1.0 + psi1 * v + psi2 * v2) / (
        v2 * v
    )


def IMRPhenomX_Return_MSA_Corrections_MSA(v, LNorm, JNorm, pPrec):
    v2 = v * v
    ...
    A_theta_L = 0.5 * (
        (JNorm / LNorm) + (LNorm / JNorm) - (pPrec.Spl2 / (JNorm * LNorm))
    )
...
def IMRPhenomX_Return_Psi_MSA(v, v2, pPrec):
    return (
        ...
        * pPrec.g0
        * pPrec.delta_qq
        * (1.0 + pPrec.psi1 * v + pPrec.psi2 * v2)
        / (v2 * v)
    )
...
def IMRPhenomX_Return_Constants_c_MSA(v, JNorm, pPrec):
    ...
    x = JNorm * (
        ...
        * (
            ...
            * (
                JNorm2
                - pPrec.Spl2
                + 2.0 * (pPrec.S1_norm_2 - pPrec.S2_norm_2) * pPrec.delta_qq
            )
            * v2
    ...
    z = JNorm * (
        0.75 * pPrec.inveta * (pPrec.Spl2 - pPrec.Smi2) ** 2 * (1.0 - Seff * v) * v6
    )
...
def IMRPhenomX_Return_Constants_d_MSA(LNorm, JNorm, pPrec):
    ...
    x = -jnp.multiply(
        JNorm2 - jnp.square(LNorm + pPrec.Spl), JNorm2 - jnp.square(LNorm - pPrec.Spl)
    )
...
def IMRPhenomX_Return_phiz_MSA(v: float, JNorm: float, pPrec) -> float:
    invv = 1.0 / v
    invv2 = invv * invv
    LNewt = pPrec.eta / v
    ...
    phiz_0_coeff = (JNorm * pPrec.inveta**4) * (
        0.5 * c12
        - (c1 * pPrec.eta2 * invv) / 6.0
        - (SAv2 * pPrec.eta2) / 3.0
        - (pPrec.eta4 * invv2) / 3.0
    ) - (0.5 * c1 * pPrec.inveta) * (
...
def IMRPhenomX_Return_zeta_MSA(v: float, pPrec) -> float:
    invv = 1.0 / v
    invv2 = invv * invv
    invv3 = invv * invv2
    v2 = v * v
...
def get_deltaF_from_wfstruct(pWF: dict) -> float:
    ...
    deltaFv1 = 1.0 / jnp.maximum(
        4.0, jnp.power(2, jnp.ceil(jnp.log(seglen) / jnp.log(2)))
    )
...
def XLALSimInspiralChirpTimeBound(
    fstart: float, m1: float, m2: float, s1: float, s2: float
) -> float:
    """
...
def check_kerr_bound(pnr_use_tuned_angles, pnr_single_spin, chi1_norm, chi2_norm):
    ...
    should_check = jnp.logical_or(
        jnp.logical_not(pnr_use_tuned_angles), pnr_single_spin != 1
    )
    ...
    error_condition = jnp.logical_and(
        should_check, jnp.logical_or(chi1_violation, chi2_violation)
    )
...
def IMRPhenomX_InspiralAngles_SpinTaylor(
    ...
    deltaT_coarse = 0.5 * lalParams["coarse_fac"] / fCut
    ...
    PhenomXPInspiralArrays = jax.lax.cond(
        fref_zero_or_same_to_fmin,
        integrate_forward,
    ...
    fminPN = jnp.power(PhenomXPInspiralArrays[0][0], 3.0) / piGM
...
def integrate_both_sides(
    ...
    LNhaty = jnp.append(LNhaty, LNhaty_forward)
...
def XLALSimIMRPhenomXPrecessingFinalSpin2017(
    eta: float, chi1L: float, chi2L: float, chi_perp: float
) -> float:
    """
...
def IMRPhenomX_SetPrecessingRemnantParams(pPrec, pWF: dict, lalParams: dict):
    """
    ...
    PNRUseInputCoprecDeviations = (
        False  # pPrec.lalParams.get('IMRPhenomXPNRUseInputCoprecDeviations', False)
    )
    ...
    def case_3():
        ...
        def msa_case():
            ...
            def fallback():
                return XLALSimIMRPhenomXPrecessingFinalSpin2017(
                    pWF["eta"], chi1L, chi2L, pPrec.chi_p
                )
    ...
    if not PNRUseTunedCoprec:
        ...
        pWF["afinal"] = (
            pnr_window * pWF["afinal_nonprec"] + (1.0 - pnr_window) * afinal_prec
        )

    # Enforce Kerr bound on final spins
    pWF["afinal"] = jax.lax.select(
        jnp.abs(afinal) > 1.0, jnp.copysign(1.0, pWF["afinal"]), pWF["afinal"]
    )

    afinal_prec = jax.lax.select(
        jnp.abs(afinal_prec) > 1.0, jnp.copysign(1.0, afinal_prec), afinal_prec
    )
    ...
    if APPLY_PNR_DEVIATIONS:
        pWF["fRING"] = pWF["fRING"] - pWF.get("PNR_DEV_PARAMETER", 0.0) * pWF.get(
            "NU5", 0.0
        )
        pWF["fDAMP"] = pWF["fDAMP"] + pWF.get("PNR_DEV_PARAMETER", 0.0) * pWF.get(
            "NU6", 0.0
        )
    ...
    if PNRUseTunedCoprec and (pWF.get("PNR_SINGLE_SPIN", 0) != 1):
        ...
        fRINGEffShiftDividedByEmm = (1.0 - jnp.abs(jnp.cos(pWF["betaRD"]))) * (
            fRING22_prec - fRING21_prec
        )
        ...
        pnr_window = pWF.get("pnr_window", 1.0)
        pWF["fRING"] = (
            pWF["fRING"] - (1.0 - pnr_window) * emm * fRINGEffShiftDividedByEmm
        )

    return Mfinal, afinal, fRING, fDAMP


def IMRPhenomX_Initialize_Euler_Angles(
    pWF: dict, pPrec: IMRPhenomXGetAndSetPrecessionVariables, lalParams: dict
):
    """
    Wrapper of IMRPhenomX_SpinTaylorAnglesSplinesAll:
    fmin and fmax are determined by the function based on the mode content and binary's parameters .
    Used in LALSimIMRPhenomXPHM.c
    """

    # Currently lalParams['PhenomXPHMThresholdMband'] = 0 TODO: add PhenomXPHMThresholdMband
    thresholdPMB = 0

    # See if PNR angles were turned off in pPrec check
    buffer = pPrec.integration_buffer

    # start below fMin to avoid interpolation artefacts
    fminAngles = (pWF["fMin"] - buffer) * 2.0 / pPrec.M_MAX
    # check we still pass a meaningful fmin TODO

    # If MB is on, we take advantage of the fact that we can compute angles on an array
    Mfmax_angles = jax.lax.select(
        thresholdPMB > 0.0,
        pWF["fRING"] + 4.0 * pWF["fDAMP"],
        (
            jnp.max(pWF["MfMax"], pWF["fRING"] + 4.0 * pWF["fDAMP"])
            + XLALSimIMRPhenomXUtilsHztoMf(buffer, pWF["Mtot"])
        )
        * 2.0
        / pPrec.M_MIN,
    )
    object.__setattr__(pPrec, "Mfmax_angles", Mfmax_angles)
    fmaxAngles = XLALSimIMRPhenomXUtilsMftoHz(pPrec.Mfmax_angles, pWF["Mtot"])

    # Build splines for alpha and cosbeta in the PN freq. range and for gamma between fminAngles and fmaxAngles
    IMRPhenomX_SpinTaylorAnglesSplinesAll(fminAngles, fmaxAngles, pWF, pPrec, lalParams)

    # evaluate and store splines at ftrans_MRD
    alpha_ftrans = pPrec.alpha_spline(pPrec.ftrans_MRD)
    cosbeta_ftrans = pPrec.cosbeta_spline(pPrec.ftrans_MRD)
    gamma_ftrans = pPrec.gamma_spline(pPrec.ftrans_MRD)

    object.__setattr__(pPrec, "alpha_ftrans", alpha_ftrans)
    object.__setattr__(pPrec, "cosbeta_ftrans", cosbeta_ftrans)
    object.__setattr__(pPrec, "gamma_ftrans", gamma_ftrans)


def gamma_from_alpha_cosbeta(
    Mf: float, deltaMf: float, pPrec: IMRPhenomXGetAndSetPrecessionVariables
) -> float:
    """
    ...
    Mf_high = Mf + deltaMf
    step = (Mf_high - Mf) * 0.25

    def Mf_under_ftrans_MRD(Mf_aux):
        cosbeta_aux = pPrec.cosbeta_spline(Mf_aux)
        alphadoti = pPrec.alpha_spline(Mf_aux, dx=1)
        alphadotcosbeta = cosbeta_aux * alphadoti
        return alphadotcosbeta

    def Mf_over_ftrans_MRD(Mf_aux):
        cosbeta_aux = jnp.cos(betaMRD(Mf_aux, pPrec.beta_params))
        alphadoti = jax.grad(alphaMRD)(Mf_aux, pPrec.alpha_params)
        alphadotcosbeta = cosbeta_aux * alphadoti
        return alphadotcosbeta

    Mf_series = jnp.arange(Mf, Mf_high + 1e-10, step)
    alphadotcosbeta = jnp.where(
        Mf_series < pPrec.ftrans_MRD,
        Mf_under_ftrans_MRD(Mf_series),
        Mf_over_ftrans_MRD(Mf_series),
    )
    ...
    gamma = (
        ...
        * step
        / 45.0
        * (
            7.0 * alphadotcosbeta[0]
            + 32.0 * alphadotcosbeta[1]
            + 12.0 * alphadotcosbeta[2]
            + 32.0 * alphadotcosbeta[3]
            + 7.0 * alphadotcosbeta[4]
        )
    )
...
def IMRPhenomX_SpinTaylorAnglesSplinesAll(
    fmin: float,
    fmax: float,
    pWF: dict,
    ...
    PNarrays, _ = (
        IMRPhenomX_InspiralAngles_SpinTaylor(  # Should we carry PNarrays as an attribute of pPrec?
            pPrec.chi1x,
            pPrec.chi1y,
            pPrec.chi1z,
            pPrec.chi2x,
    ...
    IMRPhenomX_InterpolateAlphaBeta_SpinTaylor(pWF, pPrec, lalParams, PNarrays)

    # Evaluate splines for gamma
    IMRPhenomX_InterpolateGamma_SpinTaylor(fmin, fmax, pWF, pPrec)

    # check if we can evaluate alpha(fref) using PN or an analytical continuation
    alpha_ref = jax.lax.cond(
        MfRef <= pPrec.ftrans_MRD,
        lambda args: args[1].alpha_spline(args[0]),
        lambda args: jax.lax.cond(
            (args[1].IMRPhenomXPrecVersion == 320)
            | (args[1].IMRPhenomXPrecVersion == 321),
            lambda args: alphaMRD(args[0], args[1].alpha_params),
            lambda args: args[1].alpha_spline(args[1].ftrans_MRD),
            operand=args,
        ),
        operand=(MfRef, pPrec),
    )
    object.__setattr__(pPrec, "alpha_ref", alpha_ref)


def IMRPhenomX_InterpolateGamma_SpinTaylor(
    fmin: float,  # starting frequency (Hz)
    fmax: float,  # maximum frequency (Hz)
    pWF: dict,  # [in] Waveform structure [in]
    pPrec: IMRPhenomXGetAndSetPrecessionVariables,  # [in] Precession structure [in]
):
    # some useful quantities in geom units
    Mfmin = XLALSimIMRPhenomXUtilsHztoMf(fmin, pWF["Mtot"])

    deltaMF = get_deltaF_from_wfstruct(pWF)
    deltaF = XLALSimIMRPhenomXUtilsMftoHz(deltaMF, pWF["Mtot"])

    output_length = (
        (fmax - fmin) / deltaF + 1
    )  # needs to be >=4 for spline interpolation of gamma, TODO: add check?

    # Initialize frequency array
    frequencies = Mfmin + jnp.arange(output_length) * deltaMF

    Mf_array = frequencies[1:]
    delta_gamma_array = jax.vmap(gamma_from_alpha_cosbeta, in_axes=(0, None, None))(
        Mf_array, deltaMF, pPrec
    )

    # Apply the MRD transition logic
    in_inspiral = Mf_array < pPrec.ftrans_MRD
    use_mrd_continuation = jnp.logical_or(
        pPrec.IMRPhenomXPrecVersion == 320, pPrec.IMRPhenomXPrecVersion == 321
    )

    delta_gamma_array = jnp.where(
        jnp.logical_or(in_inspiral, use_mrd_continuation), delta_gamma_array, 0.0
    )

    # Compute cumulative sum to get gamma values
    # gamma[0] = 0, gamma[i] = sum(delta_gamma[1:i])
    gamma_cumsum = jnp.cumsum(delta_gamma_array)
    gamma_array = jnp.concatenate([jnp.array([0.0]), gamma_cumsum])

    gamma_spline = Interpolator1D(
        frequencies, gamma_array, method="cubic", extrap=False
    )
    gamma_ref = gamma_spline(pWF["MfRef"])

    object.__setattr__(pPrec, "gamma_spline", gamma_spline)
    object.__setattr__(pPrec, "gamma_ref", gamma_ref)


def IMRPhenomX_InterpolateAlphaBeta_SpinTaylor(
    pWF: dict,
    pPrec: IMRPhenomXGetAndSetPrecessionVariables,
    lalParams: dict,
    PNarrays: Array,
    ...
    V_PN, Phi, S1x, S1y, S1z, S2x, S2y, S2z, LNhatx, LNhaty, LNhatz, E1x, E1y, E1z = (
        PNarrays
    )
    lenPN = V_PN.shape[0]

    LNhat_temp = jnp.array([LNhatx, LNhaty, LNhatz])
    LNhat_temp = IMRPhenomX_rotate_z(-pPrec.phiJ_Sf, LNhat_temp)
    LNhat_temp = IMRPhenomX_rotate_y(-pPrec.thetaJ_Sf, LNhat_temp)
    LNhat_temp = IMRPhenomX_rotate_z(-pPrec.kappa, LNhat_temp)

    fgw_Hz = V_PN**3 / pPrec.piGM
    fgw_Mf = XLALSimIMRPhenomXUtilsHztoMf(fgw_Hz, pWF["Mtot"])

    alphaaux = jnp.where(fgw_Hz > 0.0, jnp.atan2(LNhat_temp[1], LNhat_temp[0]), 0.0)
    cosbetaaux = jnp.where(fgw_Hz > 0.0, LNhat_temp[2], 1.0)

    fgw = jnp.where(fgw_Hz > 0.0, fgw_Mf, 0.0)
    Mfmax_PN = Mfmax_PN = jnp.max(jnp.where(fgw_Hz > 0.0, fgw_Mf, -jnp.inf))
    # i_max is used to discard possibly unphysical points in the calculation of the final spin
    i_max = jnp.argmax(jnp.where(fgw_Hz > 0.0, fgw_Mf, -jnp.inf))

    fmax_inspiral = jax.lax.select(
        lalParams["PNRUseTunedAngles"], Mfmax_PN, Mfmax_PN - pWF["deltaMF"]
    )

    fmax_inspiral = jax.lax.select(
        fmax_inspiral > pWF["fRING"] - pWF["fDAMP"], 1.020 * pWF["fMECO"], fmax_inspiral
    )

    ftrans_MRD = 0.98 * fmax_inspiral
    object.__setattr__(pPrec, "ftrans_MRD", ftrans_MRD)
    object.__setattr__(pPrec, "fmax_inspiral", fmax_inspiral)

    # Interpolate alpha
    alpha = jnp.unwrap(
        alphaaux
    )  # XLALSimIMRPhenomXUnwrapArray(alphaaux->data, alpha->data, lenPN), line 4466

    alpha_spline = Interpolator1D(fgw, alpha, method="cubic", extrap=False)
    object.__setattr__(pPrec, "alpha_spline", alpha_spline)

    # Interpolate cosbeta
    cosbeta_spline = Interpolator1D(fgw, cosbetaaux, method="cubic", extrap=False)
    object.__setattr__(pPrec, "cosbeta_spline", cosbeta_spline)

    cosbetamax = cosbeta_spline(fmax_inspiral)

    # Estimate final spin using spins at the end of the PN integration
    def XPinalSpinMod_is_4():
        m1 = pWF["m1_SI"] / pWF["Mtot_SI"]
        m2 = pWF["m2_SI"] / pWF["Mtot_SI"]

        Lnf = jnp.array([LNhatx[i_max], LNhaty[i_max], LNhatz[i_max]])
        Lnorm = jnp.linalg.norm(Lnf)
        S1f = jnp.array([S1x[i_max], S1y[i_max], S1z[i_max]])
        S2f = jnp.array([S2x[i_max], S2y[i_max], S2z[i_max]])

        dotS1L = jnp.dot(S1f, Lnf) / Lnorm
        dotS2L = jnp.dot(S2f, Lnf) / Lnorm
        S1_perp = S1f - Lnf * dotS1L
        S1_perp = S1_perp * m1 * m1
        S2_perp = S2f - Lnf * dotS2L
        S2_perp = S2_perp * m2 * m2
        Stot_perp = S1_perp + S2_perp
        S_perp_norm = jnp.sqrt(jnp.dot(Stot_perp, Stot_perp))
        chi_perp_norm = S_perp_norm * (m1 + m2) ** 2 / m1**2

        afinal = jnp.copysign(
            1.0, cosbetamax
        ) * XLALSimIMRPhenomXPrecessingFinalSpin2017(
            pWF["eta"], dotS1L, dotS2L, chi_perp_norm
        )

        fRING = evaluate_QNMfit_fring22(afinal) / (pWF["Mfinal"])
        fDAMP = evaluate_QNMfit_fdamp22(afinal) / (pWF["Mfinal"])
        return afinal, fRING, fDAMP

    afinal, fRING, fDAMP = jax.lax.cond(
        lalParams["PhenomXPFinalSpinMod"] == 4,
        XPinalSpinMod_is_4,
        lambda: (pWF["afinal_nonprec"], pWF["fRING"], pWF["fDAMP"]),
        operand=None,
    )

    object.__setattr__(pWF, "afinal", afinal)
    object.__setattr__(pWF, "fRING", fRING)
    object.__setattr__(pWF, "fDAMP", fDAMP)

    # For versions 320, 321, 330, initialize parameters for RD continuation
    def versions_withRDcontinuation():
        alpha_params = alphaMRD_coeff(alpha_spline, fmax_inspiral)
        beta_params = betaMRD_coeff(cosbeta_spline, fmax_inspiral, pWF, pPrec)

        object.__setattr__(pPrec, "alpha_params", alpha_params)
        object.__setattr__(pPrec, "beta_params", beta_params)

    has_RD_continuation = jnp.logical_or(
        pPrec.IMRPhenomXPrecVersion == 330,
        jnp.logical_or(
            pPrec.IMRPhenomXPrecVersion == 320, pPrec.IMRPhenomXPrecVersion == 321
        ),
    )
    ...
    jax.lax.cond(
        has_RD_continuation, versions_withRDcontinuation, lambda: None, operand=None
    )


def alphaMRD_coeff(spline_alpha: Interpolator1D, fmaxPN: float):
    """
    Analytical continuation for alpha angle in MRD
    3878-3933 in LALSimIMRPhenomX_precession.c
    """
    ...
    aC = (
        f1cube * (f1 - f2) * (f1 + f2) * dalpha1
        + 2 * (f1**4 - 2 * f1sq * f2sq) * alpha1
        + 2 * f2**4 * alpha2
    ) / (2.0 * (f1sq - f2sq) ** 2)
    ...
    bC = (
        f1**4
        * f2sq
        * (f1 * (f1 - f2) * (f1 + f2) * dalpha1 + 2 * f2sq * (-alpha1 + alpha2))
    ) / (2.0 * (f1sq - f2sq) ** 2)

    cC = (
        f1sq * (f1 * (-(f1**4) + f2**4) * dalpha1 + 4 * f2**4 * (alpha1 - alpha2))
    ) / (2.0 * (f1sq - f2sq) ** 2)

    return dict(aRD=aC, bRD=bC, cRD=cC)


def alphaMRD(Mf: float, alpha_params: dict) -> float:
    Mf2 = Mf * Mf
    Mf4 = Mf2 * Mf2
    minalpha = (
        alpha_params["aRD"] + alpha_params["bRD"] / Mf4 + alpha_params["cRD"] / Mf2
    )
...
def betaMRD_coeff(
    ...
    dfdamp = pPrec.beta_params["dfdamp"]  # placeholder until QNMs are added
    kappa = 2.0 * PI * dfdamp
    ...
    cosbeta1 = spline_cosb(f1)

    dcosbeta2 = spline_cosb(f2, dx=1)

    cosbeta2 = spline_cosb(f2)

    cosbetamax = spline_cosb(pPrec.fmax_inspiral)
    ...
    def flatRD_branch():
        ...
        cC = 0.0
        dC = 0.0
        return aC, bC, cC, dC, True

    def non_flatRD_branch():
        beta1 = jnp.acos(cosbeta1)
        beta2 = jnp.acos(cosbeta2)
        sqrtarg = 1.0 - cosbeta2 * cosbeta2
        dbeta2 = -dcosbeta2 / jnp.sqrt((jnp.where(sqrtarg <= 0.0, 1.0, sqrtarg)))

        off = jax.lax.cond(
            pPrec.IMRPhenomXPrecVersion == 330,
            IMRPhenomX_PNR_GenerateRingdownPNRBeta,
            lambda _: (jnp.where(cosbetamax < 0.0, PI, 0.0)),
            operand=(pWF, pPrec),
        )
        ...
        aC = (
            ...
            * (
                f2 * (-f1 + f2) * dbeta2
                + (-(f2 * (3 + f2 * kappa)) + f1 * (4 + f2 * kappa)) * (off - beta2)
            )
        ) / (f1 - f2) ** 2
        bC = (
            2 * ef1 * f1**4 * f2 * (off - beta1)
            + ef2
            * f2**3
            * (
                (f1 - f2) * f2 * (f1 + f2) * dbeta2
                - (-(f2sq * (2 + f2 * kappa)) + f1sq * (4 + f2 * kappa)) * (off - beta2)
            )
        ) / (f1 - f2) ** 2
        cC = (
            ...
            * (
                f2 * (-f1 + f2) * dbeta2
                + (-(f2 * (2 + f2 * kappa)) + f1 * (3 + f2 * kappa)) * (off - beta2)
            )
        ) / (f1 - f2) ** 2
        dC = off

        return aC, bC, cC, dC, False

    use_flatRD = jnp.logical_or(
        jnp.logical_or(jnp.fabs(cosbeta1) > 1.0, jnp.fabs(cosbeta2) > 1.0),
        jnp.fabs(cosbetamax) > 1.0,  # FIXME: add error handling to this
    )

    aC, bC, cC, dC, flat_RD = jax.lax.cond(
        use_flatRD, flatRD_branch, non_flatRD_branch, operand=None
    )

    cosbeta_sign = jnp.copysign(1.0, cosbetamax)
...
def betaMRD(Mf: float, beta_params: dict) -> float:
    def flat_RD_branch():
        return jnp.acos(beta_params["cosbeta_sign"])

    def non_flat_RD_branch():
        kappa = 2.0 * PI * beta_params["dfdamp"]
        Mf2 = Mf * Mf
        Mf3 = Mf2 * Mf
        beta = (
            jnp.exp(-Mf * kappa)
    ...
    return jax.lax.cond(
        beta_params["flat_RD"], flat_RD_branch, non_flat_RD_branch, operand=None
    )


def IMRPhenomX_PNR_GenerateRingdownPNRBeta(
    pWF: dict, pPrec: IMRPhenomXGetAndSetPrecessionVariables
) -> float:
    ...
    betafinal = IMRPhenomX_PNR_arctan_window(
        jnp.arccos(costhetaf) - IMRPhenomX_PNR_beta_Bf_coefficient(eta, chi, costheta)
    )
