"""Implementations of spin -2 weighted spherical harmonics for specific l and m values."""
...
def compute_sminus2_l2(theta, m):
    ...
    harmonics = jnp.where(
        ...
        jnp.where(
            ...
            jnp.where(
                m == 0,
                jnp.sqrt(15.0 / (32.0 * jnp.pi)) * jnp.sin(theta) ** 2,
                jnp.where(
                    m == 1,
                    jnp.sqrt(5.0 / (16.0 * jnp.pi)) * jnp.sin(theta) * (1.0 + jnp.cos(theta)),
                    jnp.sqrt(5.0 / (64.0 * jnp.pi)) * (1.0 + jnp.cos(theta)) ** 2,
...
def compute_sminus2_l3(theta, m):
    ...
    harmonics = jnp.where(
        m == -3,
        jnp.sqrt(21.0 / (2.0 * jnp.pi)) * jnp.cos(theta / 2.0) * (jnp.sin(theta / 2.0) ** 5),
        jnp.where(
            m == -2,
            jnp.sqrt(7.0 / (4.0 * jnp.pi)) * (2.0 + 3.0 * jnp.cos(theta)) * (jnp.sin(theta / 2.0) ** 4),
            jnp.where(
                m == -1,
                jnp.sqrt(35.0 / (2.0 * jnp.pi))
                * (jnp.sin(theta) + 4.0 * jnp.sin(2.0 * theta) - 3.0 * jnp.sin(3.0 * theta))
                / 32.0,
                jnp.where(
                    m == 0,
                    (jnp.sqrt(105.0 / (2.0 * jnp.pi)) * jnp.cos(theta) * (jnp.sin(theta) ** 2)) / 4.0,
                    jnp.where(
                        m == 1,
                        -jnp.sqrt(35.0 / (2.0 * jnp.pi))
                        * (jnp.sin(theta) - 4.0 * jnp.sin(2.0 * theta) - 3.0 * jnp.sin(3.0 * theta))
                        ...
                        jnp.where(
                            m == 2,
                            jnp.sqrt(7.0 / jnp.pi) * (jnp.cos(theta / 2.0) ** 4) * (-2.0 + 3.0 * jnp.cos(theta)) / 2.0,
...
def compute_sminus2_l4(theta, m):
    ...
    harmonics = jnp.where(
        m == -4,
        3.0 * jnp.sqrt(7.0 / jnp.pi) * (jnp.cos(theta / 2.0) ** 2) * (jnp.sin(theta / 2.0) ** 6),
        jnp.where(
            m == -3,
            3.0
