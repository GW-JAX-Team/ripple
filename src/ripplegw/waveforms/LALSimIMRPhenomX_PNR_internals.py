...
def XLALSimIMRPhenomXFinalSpin2017(eta: float, chi1L: float, chi2L: float) -> float:
    ...
    uneqSpin = (
        ...
        * dchi
        * delta
        * (1 + 9.332575956437443 * eta)
        * eta2  # Linear in spin difference
...
def IMRPhenomX_PNR_GetAndSetPNRVariables(pPrec, pWF: dict):
    ...
    chiperp, chiperp_antisymmetric = evaluate_chiperp(
        m1, m2, chi1x, chi1y, chi2x, chi2y, pPrec.chi_p, pPrec.IMRPhenomXPrecVersion
    )

    # Get the total magnitude, Eq. 18 of arXiv:2107.08876
    chi_mag = jnp.sqrt(chipar * chipar + chiperp * chiperp)
    chi_mag_antisymmetric = jnp.sqrt(
        chipar * chipar + chiperp_antisymmetric * chiperp_antisymmetric
    )
    ...
    theta_antisymmetric = jnp.where(
        chi_mag_antisymmetric >= 1.0e-6, jnp.arccos(chipar / chi_mag_antisymmetric), 0.0
    )
    ...
    PNRInspiralScaling = jnp.where(jnp.logical_or(q > 12.0, chi_mag > 1.2), 1, 0)
    object.__setattr__(pPrec, "PNRInspiralScaling", PNRInspiralScaling)

    return pPrec  # XLAL_SUCCESS


def IMRPhenomX_PNR_HMInterpolationDeltaF(f_min: float, pWF: dict, pPrec) -> float:
    """
    ...
    aligned_spin_condition = jnp.logical_and(
        pPrec.chi1_perp == 0.0, pPrec.chi2_perp == 0.0
    )
    ...
    def precessing_case():
        ...
        deltaF_alpha = XLALSimPhenomUtilsMftoHz(
            constant * jnp.sqrt(jnp.sqrt(numerator / denominator)), pWF["Mtot"]
        )
        ...
        def handle_two_spin():
            precessing_tag = (
                pPrec.IMRPhenomXPrecVersion - (pPrec.IMRPhenomXPrecVersion % 100)
            ) // 100
            ...
            def setup_spintaylor():
                ...
                L3 = (
                    5
                    * (chi1L * (-2 - 2 * delta + eta) + chi2L * (-2 + 2 * delta + eta))
                ) / 6.0
                ...
                L5 = (
                    ...
                    * (
                        ...
                        + chi2L
                        * (72 + eta * (-121 + 2 * eta) + delta * (-72 + 31 * eta))
                    )
                ) / 144.0
                L6 = (
                    10935
                    + eta
                    * (-62001 + eta * (1674 + 7 * eta) + 2214 * jnp.power(jnp.pi, 2))
                ) / 1296.0
                L7 = 0.0
                L8 = 0.0
                ...
                object.__setattr__(pPrec, "L0", L0)
                object.__setattr__(pPrec, "L1", L1)
                object.__setattr__(pPrec, "L2", L2)
                object.__setattr__(pPrec, "L3", L3)
                object.__setattr__(pPrec, "L4", L4)
                object.__setattr__(pPrec, "L5", L5)
                object.__setattr__(pPrec, "L6", L6)
                object.__setattr__(pPrec, "L7", L7)
                object.__setattr__(pPrec, "L8", L8)
                object.__setattr__(pPrec, "L8L", L8L)

                user_version = pPrec.IMRPhenomXPrecVersion

                object.__setattr__(pPrec, "IMRPhenomXPrecVersion", 223)

                IMRPhenomX_Initialize_MSA_System(
                    pPrec, pWF, pPrec.lalParams["ExpansionOrder"]
                )

                object.__setattr__(pPrec, "IMRPhenomXPrecVersion", user_version)
                return
            ...
            betaMin = jnp.arctan2(
                jnp.abs(pPrec.S1_perp - pPrec.S2_perp), L_fmin + pPrec.SL
            )
            betaMax = jnp.arctan2(pPrec.S1_perp + pPrec.S2_perp, L_fmin + pPrec.SL)

            # Adjust dpsiInv if conditions are met
            large_oscillation_condition = jnp.logical_and(
                betaMin < 0.01, betaMin / betaMax < 0.55
            )
            ...
            use_twospin_condition = jnp.logical_and(
                deltaF_twospin < deltaF_alpha, jnp.logical_not(jnp.isnan(dpsi))
            )
...
def IMRPhenomX_PNR_CheckTwoSpin(pPrec) -> bool:
    """
    ...
    condition = jnp.logical_and(
        jnp.logical_and(pPrec.chi1_norm != 0.0, pPrec.chi2_norm >= 1.0e-3),
...
def evaluate_chiperp(m1, m2, chi1x, chi1y, chi2x, chi2y, chi_p, IMRPhenomXPrecVersion):
    ...
    chiperp_antisymmetric_weighted = (
        sin_term * sin_term * chi_p + cos_term * cos_term * antisymmetric_chis
    )
...
def IMRPhenomX_PNR_GetAndSetCoPrecParams(pPrec, pWF: dict, lalParams: dict):
    ...
    PNRUseTunedCoprec33 = (
        False  # lalParams.get('PNRUseTunedCoprec33', False) and PNRUseTunedCoprec
    )
    ...
    PNRUseInputCoprecDeviations = (
        False  # lalParams.get('PNRUseInputCoprecDeviations', False)
    )
    object.__setattr__(
    ...
    if PNRUseTunedCoprec:
        ...
        coprec_eta = jnp.where(
            ...
            pWF["eta"],
            jnp.where(
                is_version_330, 0.09876 - (0.09876 - pWF["eta"]) * 0.1641, 0.09876
            ),
        )

        # Flatten spin dependence
        coprec_a1 = jnp.where(
            pWF["a1"] <= 0.8, pWF["a1"], 0.8 + (pWF["a1"] - 0.8) / 12.0
        )
        coprec_a1 = jnp.where(
            is_version_330, coprec_a1, jnp.where(pWF["a1"] <= 0.8, pWF["a1"], 0.8)
        )
