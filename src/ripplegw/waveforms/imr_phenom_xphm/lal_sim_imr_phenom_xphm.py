...
def check_mass_ratio(mass_ratio: float) -> float:
    ...
    def warn_extrapolation(mass_ratio: float) -> float:
        ...
        """
        jax.debug.print(
            "Warning: Mass ratio = {mass_ratio}. Extrapolating outside of "
            "Numerical Relativity calibration domain. NNLO angles may become "
            "pathological at large mass ratios.",
...
def xlal_sim_imr_phenom_xphm(
    ...
    It computes the plus and cross polarizations of the multimode precessing
    waveform for positive frequencies in an equally spaced grid.
    """
    # Set initial values of masses and z-components of spins to pass to
    # imr_phenom_x_set_waveform_variables() so it can swap the matter parameters
    # (masses and spins) appropriately if m1 < m2, since the masses and spin
    # vectors will also be swapped by xlal_imr_phenom_xp_check_masses_and_spins()
    # below.
    # m1_si_init = m1_si
    # m2_si_init = m2_si
    # chi1z_init = chi1z
    ...
    # To interface with ripple, the frequency array should probably be created
    # outside and passed as an argument
    ...
    # status = IMRPhenomXPHM_hplushcross(hptilde, hctilde, freqs, pWF, pPrec,
    #                                     lalParams_aux)
    # XLAL_CHECK(status == XLAL_SUCCESS, XLAL_EFUNC,
    #            "IMRPhenomXPHM_hplushcross failed to generate IMRPhenomXHM "
    #            "waveform.\n")
    ...
    #     Resize the frequency series to fill with zeros beyond the cutoff
    #     frequency. */
    #     lastfreq = pWF->fMax
    #     XLAL_PRINT_WARNING(
    #         "The input f_max = %.2f Hz is larger than the internal cutoff of "
    #         "Mf=0.3 (%.2f Hz). Array will be filled with zeroes between these "
...
# def xlal_sim_imr_phenom_xphm_frequency_sequence_one_mode(
#     h_lm_pos: Array,
#     h_lm_neg: Array,
#     freqs: Array,
#     l: int,
