...
class IMRPhenomXPrecessionDataClass:  # pylint: disable=too-many-instance-attributes
    ...
    J_Norm_N: float = 0.0 # Norm of total angular momentum using Newtonian orbital angular momentum \f$ \left| J_{N} \right| \f$
    ...
    Seff: float = 0.0 # \f$ S_{\rm{eff}} = (1 + q^{-1}) S_1 \cdot \hat{L} + (1 + q) S_2 \cdot \hat{L} \f$, Eq. 7 of arXiv:1703.03967. Note convention for q. */
    ...
    S1L_pav: float = 0.0 # Precession averaged coupling \f$ \langle S_1 \cdot \hat{L} \rangle_{\rm{pr}} \f$, Eq. A9 of arXiv:1703.03967
    S2L_pav: float = 0.0 # Precession averaged coupling \f$ \langle S_2 \cdot \hat{L} \rangle_{\rm{pr}} \f$, Eq. A10 of arXiv:1703.03967
    S1S2_pav: float = 0.0 # Precession averaged coupling \f$ \langle S_1 \cdot S_2 \rangle_{\rm{pr}} \f$, Eq. A11 of arXiv:1703.03967
    S1Lsq_pav: float = 0.0 # Precession averaged coupling \f$ \langle (S_1 \cdot \hat{L})^2 \rangle_{\rm{pr}} \f$, Eq. A12 of arXiv:1703.03967
    S2Lsq_pav: float = 0.0 # Precession averaged coupling \f$ \langle (S_2 \cdot \hat{L})^2 \rangle_{\rm{pr}} \f$, Eq. A13 of arXiv:1703.03967
    S1LS2L_pav: float = 0.0 # Precession averaged coupling \f$ \langle (S_1 \cdot \hat{L}) (S_2 \cdot \hat{L}) \rangle_{\rm{pr}} \f$, Eq. A14 of arXiv:1703.03967 |
    ...
    alpha_ftrans: float = 0.0 # Record value of alpha at end of inspiral, used in IMRPhenomXPHMTwistUp and IMRPhenomXPHMTwistUpOneMode
    cosbeta_ftrans: float = 0.0 # Record value of cosbeta at end of inspiral, used in IMRPhenomXPHMTwistUp and IMRPhenomXPHMTwistUpOneMode
    gamma_ftrans: float = 0.0 # Record value of gamma at end of inspiral, used in IMRPhenomXPHMTwistUp and IMRPhenomXPHMTwistUpOneMode
    ...
    integration_buffer: float = 0.0 # Buffer region for integration of SpinTaylor equations: added so that interpolated angles cover the frequency range requested by user
    fmin_integration: float = 0.0 # Minimum frequency covered by the integration of PN spin-precessing equations for SpinTaylor models  
    Mfmin_integration: float = 0.0 # Minimum frequency covered by the integration of PN spin-precessing equations for SpinTaylor models  

    MSA_ERROR: float = 0.0 # Flag to track errors in initialization of MSA system. 

    # /* PNR-specific additions for single-spin mapping */
    chi_singleSpin: float = 0.0 # Magnitude of effective single spin used for tapering two-spin angles, Eq. 18 of arXiv:2107.08876
    costheta_singleSpin: float = 0.0 # Polar angle of effective single spin, Eq. 19 or arXiv:2107.08876
    costheta_final_singleSpin: float = 0.0 # Polar angle of approximate final spin, see technical document FIXME: add reference
    ...
    chi_singleSpin_antisymmetric: float = 0.0 # magnitude of effective single spin of a two spin system for the antisymmetric waveform
