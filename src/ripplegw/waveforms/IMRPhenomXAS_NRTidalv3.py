...
def _gen_IMRPhenomXAS_NRTidalv3(
    ...
    phase_shift = (
        -(phifRef - phiTfRef + dphi_merger * (f_ref * M_s))
        + dphi_merger * f_Ms
        + ext_phase_contrib
    )
    ...
    h0 = (
        A_P
        * (bbh_amp + A_T)
        * jnp.exp(1.0j * ((bbh_psi + phase_shift + PI) - (psi_T + psi_SS)))
...
def IMRPhenomX_TidalPhaseDerivative(f, theta):
    ...
    threePN_dphase = (pfaN * (-c2pn + c3pn * x ** (2.0 / 3.0))) / (
        3.0 * Mf ** (4.0 / 3.0) * PI ** (1.0 / 3.0)
    )

    dphase = threePN_dphase

    dphase += (2.0 * c3p5pn * pfaN * PI ** (2.0 / 3.0)) / (3.0 * Mf ** (1.0 / 3.0))
    ...
    NRphasetermA = -(
        ...
        / (1 + d_1A * x ** (2.0 / 3.0) + d_3over2A * Mf * PI)
    )
    NRphasetermB = -(
        (
            c_NewtB
        ...
        / (1 + d_1B * x ** (2.0 / 3.0) + d_3over2B * Mf * PI)
    )
    ...
    dynk2barfunc_deriv = (s1 - 1.0) * (2.0 * PI) * s2 * exps2Mfb * exps2s3b * (
        1 / ((1 + exps2Mfb * exps2s3b) * (1 + exps2Mfb * exps2s3b))
    ) - 2.0 * PI * ((s1) - 1.0) * s2 * exps2s3 / ((1.0 + exps2s3) * (1.0 + exps2s3))
    ...
    NRTuned_dphaseA2 = (
        ...
        * (
            -5
            - x ** (2.0 / 3.0) * (3 * d_1A + 7 * n_1A)
            - 2 * Mf * (d_3over2A + 4 * n_3over2A) * PI
            - x ** (4.0 / 3.0) * (5 * d_1A * n_1A + 9 * n_2A)
            - 2
            * x ** (5.0 / 3.0)
            * (2 * d_3over2A * n_1A + 3 * d_1A * n_3over2A + 5 * n_5over2A)
            - x**2 * (7 * d_1A * n_2A + 11 * n_3A + 5 * d_3over2A * n_3over2A)
            - 2 * x ** (7.0 / 3.0) * (3 * d_3over2A * n_2A + 4 * d_1A * n_5over2A)
            - x ** (8.0 / 3.0) * (9 * d_1A * n_3A + 7 * d_3over2A * n_5over2A)
            - 2 * x**3 * (4 * d_3over2A * n_3A)
        )
    ) / (3.0 * jnp.pow(1 + d_1A * x ** (2.0 / 3.0) + d_3over2A * Mf * PI, 2))
    NRTuned_dphaseB2 = (
        c_NewtB
        * kappaB
        * dynk2barfunc
        ...
        * (
            -5
            - x ** (2.0 / 3.0) * (3 * d_1B + 7 * n_1B)
            - 2 * Mf * (d_3over2B + 4 * n_3over2B) * PI
            - x ** (4.0 / 3.0) * (5 * d_1B * n_1B + 9 * n_2B)
            - 2
            * x ** (5.0 / 3.0)
            * (2 * d_3over2B * n_1B + 3 * d_1B * n_3over2B + 5 * n_5over2B)
            - x**2 * (7 * d_1B * n_2B + 11 * n_3B + 5 * d_3over2B * n_3over2B)
            - 2 * x ** (7.0 / 3.0) * (3 * d_3over2B * n_2B + 4 * d_1B * n_5over2B)
            - x ** (8.0 / 3.0) * (9 * d_1B * n_3B + 7 * d_3over2B * n_5over2B)
            - 2 * x**3 * (4 * d_3over2B * n_3B)
        )
    ) / (3.0 * jnp.pow(1 + d_1B * x ** (2.0 / 3.0) + d_3over2B * Mf * PI, 2))
    ...
    dPNpolyA = (
        (2.0 / 3.0) * Mf ** (-1.0 / 3.0) * c_1A * PI ** (2.0 / 3.0)
        + c_3over2A * PI
        + (4.0 / 3.0) * Mf ** (1.0 / 3.0) * c_2A * PI ** (4.0 / 3.0)
        + (5.0 / 3.0) * Mf ** (2.0 / 3.0) * c_5over2A * PI ** (5.0 / 3.0)
    )
    dPNpolyB = (
        (2.0 / 3.0) * Mf ** (-1.0 / 3.0) * c_1B * PI ** (2.0 / 3.0)
        + c_3over2B * PI
        + (4.0 / 3.0) * Mf ** (1.0 / 3.0) * c_2B * PI ** (4.0 / 3.0)
        + (5.0 / 3.0) * Mf ** (2.0 / 3.0) * c_5over2B * PI ** (5.0 / 3.0)
    )

    PNTuned_dphaseA = factorA * (
        (5.0 / 3.0) * Mf ** (2.0 / 3.0) * PI ** (5.0 / 3.0) * PNpolyA
        + x ** (5.0 / 3.0) * dPNpolyA
    )
    PNTuned_dphaseB = factorB * (
    ...
    dPlancktaper = (
        dGeneral_planck_taper(Mf, Mftaperstart, Mftaperend) * plancktaperfn**2
    )

    NRTuned_dphase = (
        NRTuned_dphaseNRT * (1.0 - plancktaperfn)
        + PNTuned_dphase * plancktaperfn
        - (NRphaseNRT - PNtidalphase) * dPlancktaper
    )
...
def dGeneral_planck_taper(x, y1, y2):
    ...
    dfactor_taper = jnp.where(
        ...
        jnp.where(
            ...
            0,
            -(exp_fn)
            * (-(y2 - y1) / ((x - y1) * (x - y1)) - (y2 - y1) / ((x - y2) * (x - y2))),
        ),
    )
