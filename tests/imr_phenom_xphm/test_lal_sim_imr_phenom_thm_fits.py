...
class TestEvaluateQnmFitFdamp22:
    """Test the QNM fit for (2,2) mode damping frequency."""

    def test_spin_zero(self):
        """Test at zero spin (non-spinning case)."""
        result = evaluate_qnm_fit_fdamp22(0.0)
        # Expected value from the fit: numerator/denominator at x=0
        expected = 0.014158792290965177 / 1.0
        assert jnp.isclose(result, expected, rtol=1e-10)

    def test_spin_half(self):
        """Test at moderate spin value."""
        spin = 0.5
        result = evaluate_qnm_fit_fdamp22(spin)
        # Compute expected manually from the polynomial
        x = spin
        x2 = x * x
        x3 = x2 * x
        ...
        den = (
            1 - 2.5900842798681376 * x + 1.8952576220623967 * x2 - 0.31416610693042507 * x4 + 0.009002719412204133 * x6
        )
        expected = num / den
        assert jnp.isclose(result, expected, rtol=1e-10)
    ...
    def test_spin_max(self):
        """Test at maximum spin (a=1)."""
        result = evaluate_qnm_fit_fdamp22(1.0)
        # Should be finite and reasonable (not NaN/inf)
        assert jnp.isfinite(result)
        assert result > 0  # Damping frequency should be positive
    ...
    def test_jit_compatibility(self):
        ...
        # Create a version without checkify for JIT testing
        def evaluate_qnm_fit_fdamp22_no_check(final_dimless_spin: float) -> float:
            x2 = final_dimless_spin * final_dimless_spin
            x3 = x2 * final_dimless_spin
            x4 = x2 * x2
            x5 = x3 * x2
        ...
        jitted_func = jax.jit(evaluate_qnm_fit_fdamp22_no_check)
        final_dimless_spin = jnp.array(0.5, dtype=jnp.float64)
        result = jitted_func(final_dimless_spin)
        expected = evaluate_qnm_fit_fdamp22_no_check(final_dimless_spin)
        assert jnp.isclose(result, expected, rtol=1e-10)
    ...
    def test_gradient(self):
        ...
        grad_func = jax.grad(evaluate_qnm_fit_fdamp22_no_check)
        gradient = grad_func(jnp.array(0.5, dtype=jnp.float64))
        # Gradient should be finite (the fit is smooth)
        assert jnp.isfinite(gradient)
    ...
    def test_vmap_compatibility(self):
        ...
        spins = jnp.array([0.0, 0.5, 1.0], dtype=jnp.float64)
        vmapped_func = jax.vmap(evaluate_qnm_fit_fdamp22_no_check)
        results = vmapped_func(spins)
        # Check each result individually
        for spin, result in zip(spins, results):
            expected = evaluate_qnm_fit_fdamp22_no_check(spin)
            assert jnp.isclose(result, expected, rtol=1e-10)

    def test_numerical_stability(self):
        """Test numerical stability near boundaries."""
        # Near a=1
        result_near_max = evaluate_qnm_fit_fdamp22(0.999)
        assert jnp.isfinite(result_near_max)

        # Near a=-1
        result_near_min = evaluate_qnm_fit_fdamp22(-0.999)
        assert jnp.isfinite(result_near_min)

        # Very small spin
        result_small = evaluate_qnm_fit_fdamp22(1e-6)
        assert jnp.isfinite(result_small)
