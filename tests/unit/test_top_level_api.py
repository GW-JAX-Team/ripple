...
import ripplegw.registry as reg  # noqa: E402
from ripplegw.interfaces import Waveform  # noqa: E402
...
def test_factory_constructs_and_matches_direct():
    ...
    assert isinstance(wf, ripplegw.IMRPhenomD)
    assert wf.f_ref == 20.0

    f = jnp.arange(20.0, 60.0, 1.0)
    params = dict(
        M_c=30.0, eta=0.24, s1_z=0.1, s2_z=-0.1, d_L=400.0, phase_c=0.0, iota=0.3
    )
    got = wf(f, params)
    ref = ripplegw.IMRPhenomD(f_ref=20.0)(f, params)
...
def test_waveform_preset_is_registry_alias():
    assert ripplegw.waveform_preset is ripplegw.WAVEFORM_REGISTRY
    for name in BUILTINS:
        assert name in ripplegw.waveform_preset


def test_legacy_class_imports_still_work():
    from ripplegw import IMRPhenomD, SineGaussian
    from ripplegw.interfaces import waveform_preset

    assert waveform_preset["IMRPhenomD"] is IMRPhenomD
    assert issubclass(SineGaussian, Waveform)
